...
template <typename Protocol BOOST_ASIO_SVC_TPARAM_DEF1(
    ...
    defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
	  ...
	  typename WaitTraits = time_traits<Clock> BOOST_ASIO_SVC_TPARAM1_DEF2(
	      = deadline_timer_service<Clock, WaitTraits>)>
#else  // defined(BOOST_ASIO_HAS_BOOST_DATE_TIME)                              \
       // && defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
	  typename Clock = chrono::steady_clock,
	  typename WaitTraits = wait_traits<Clock> BOOST_ASIO_SVC_TPARAM1_DEF1(
...
template <typename Protocol BOOST_ASIO_SVC_TPARAM, typename Clock,
	  typename WaitTraits BOOST_ASIO_SVC_TPARAM1>
...
public:
	...
	basic_socket_streambuf(basic_socket_streambuf &&other)
	    : detail::socket_streambuf_io_context(other),
		  ...
		  std::move(other.socket())),
	      ec_(other.ec_), expiry_time_(other.expiry_time_)
	{
   ...
   */
	...
	basic_socket<Protocol BOOST_ASIO_SVC_TARG> &socket() { return *this; }
   ...
   */
	...
	{
		expiry_time_ =
		    traits_helper::add(traits_helper::now(), expiry_time);
	}
   ...
   */
	...
	{
		return traits_helper::subtract(expires_at(),
					       traits_helper::now());
	}
...
protected:
	int_type underflow()
	{
		...
		for (;;) {
			...
			if (traits_helper::less_than(expiry_time_,
						     traits_helper::now())) {
			...
			detail::buffer_sequence_adapter<mutable_buffer,
							mutable_buffer>
			    bufs(boost::asio::buffer(get_buffer_) +
				 putback_max);
			detail::signed_size_type bytes =
			    detail::socket_ops::recv(socket().native_handle(),
						     bufs.buffers(),
						     bufs.count(), 0, ec_);
			...
			if (detail::socket_ops::poll_read(
				socket().native_handle(), 0, timeout(), ec_) <
			    0)
				return traits_type::eof();
		}
	...
	int_type overflow(int_type c)
	{
		...
		if (put_buffer_.empty()) {
			...
			output_buffer = boost::asio::buffer(
			    pbase(), (pptr() - pbase()) * sizeof(char_type));
		}
		...
		while (output_buffer.size() > 0) {
			...
			if (!socket().native_non_blocking())
				socket().native_non_blocking(true, ec_);
			detail::buffer_sequence_adapter<const_buffer,
							const_buffer>
			    bufs(output_buffer);
			detail::signed_size_type bytes =
			    detail::socket_ops::send(socket().native_handle(),
						     bufs.buffers(),
						     bufs.count(), 0, ec_);
		...
		if (!put_buffer_.empty()) {
			setp(&put_buffer_[0],
			     &put_buffer_[0] + put_buffer_.size());
...
private:
	...
	basic_socket_streambuf(const basic_socket_streambuf &)
	    BOOST_ASIO_DELETED;
	basic_socket_streambuf &
	operator=(const basic_socket_streambuf &) BOOST_ASIO_DELETED;
	...
	int timeout() const
	{
		int64_t msec = traits_helper::to_posix_duration(
				   traits_helper::subtract(
				       expiry_time_, traits_helper::now()))
				   .total_milliseconds();
	...
	void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
	{
		...
		for (EndpointIterator i = begin; i != end; ++i) {
			...
			if (detail::socket_ops::getsockopt(
				socket().native_handle(), 0, SOL_SOCKET,
