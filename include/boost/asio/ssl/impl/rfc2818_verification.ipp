				      ...
				      verify_context &ctx) const
{
	...
	for (int i = 0; i < sk_GENERAL_NAME_num(gens); ++i) {
		...
		if (gen->type == GEN_DNS && !is_address) {
			...
			if (domain->type == V_ASN1_IA5STRING && domain->data &&
				...
				if (match_pattern(pattern, pattern_length,
						  host_.c_str())) {
			...
			if (ip_address->type == V_ASN1_OCTET_STRING &&
			    ip_address->data) {
				if (address.is_v4() &&
				    ip_address->length == 4) {
					...
					if (memcmp(bytes.data(),
					   ...
					   ip_address->length == 16) {
					ip::address_v6::bytes_type bytes =
					    address.to_v6().to_bytes();
					if (memcmp(bytes.data(),
						   ip_address->data, 16) == 0) {
						GENERAL_NAMES_free(gens);
						return true;
					}
				}
	...
	if (common_name && common_name->data && common_name->length) {
		const char *pattern =
		    reinterpret_cast<const char *>(common_name->data);
