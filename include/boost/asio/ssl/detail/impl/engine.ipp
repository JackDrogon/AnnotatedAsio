...
engine::~engine()
{
	if (SSL_get_app_data(ssl_)) {
		delete static_cast<verify_callback_base *>(
		    SSL_get_app_data(ssl_));
...
boost::system::error_code engine::set_verify_mode(verify_mode v,
						  boost::system::error_code &ec)
{
...
engine::set_verify_depth(int depth, boost::system::error_code &ec)
{
...
boost::system::error_code
engine::set_verify_callback(verify_callback_base *callback,
			    boost::system::error_code &ec)
{
	if (SSL_get_app_data(ssl_))
		delete static_cast<verify_callback_base *>(
...
engine::get_output(const boost::asio::mutable_buffer &data)
{
	int length =
	    ::BIO_read(ext_bio_, data.data(), static_cast<int>(data.size()));

	return boost::asio::buffer(
	    data, length > 0 ? static_cast<std::size_t>(length) : 0);
}

boost::asio::const_buffer
engine::put_input(const boost::asio::const_buffer &data)
{
	int length =
...
{
	static boost::asio::detail::static_mutex mutex =
	    BOOST_ASIO_STATIC_MUTEX_INIT;
	return mutex;
}
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
			     ...
			     std::size_t *bytes_transferred)
{
	...
	if (ssl_error == SSL_ERROR_SSL) {
		ec = boost::system::error_code(
		    sys_error, boost::asio::error::get_ssl_category());
		return want_nothing;
	}

	if (ssl_error == SSL_ERROR_SYSCALL) {
		ec = boost::system::error_code(
		    sys_error, boost::asio::error::get_system_category());
		return want_nothing;
	}
...
{
	return ::SSL_read(
	    ssl_, data, length < INT_MAX ? static_cast<int>(length) : INT_MAX);
}
