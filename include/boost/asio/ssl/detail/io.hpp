	...
	do
		...
		case engine::want_input_and_retry:
			...
			if (core.input_.size() == 0)
				core.input_ = boost::asio::buffer(
				    core.input_buffer_,
				    next_layer.read_some(core.input_buffer_,
							 ec));
...
public:
	...
	io_op(const io_op &other)
	    : next_layer_(other.next_layer_), core_(other.core_),
	      op_(other.op_), start_(other.start_), want_(other.want_),
	      ec_(other.ec_), bytes_transferred_(other.bytes_transferred_),
			...
			int start = 0)
	{
		...
		case 1: // Called after at least one async operation.
			do {
				switch (want_ = op_(core_.engine_, ec_,
						    bytes_transferred_)) {
				case engine::want_input_and_retry:
					...
					if (core_.expiry(core_.pending_read_) ==
						...
						core_.pending_read_.expires_at(
						    core_.pos_infin());
				...
				case engine::want_output:
					...
					if (core_.expiry(
						core_.pending_write_) ==
					    core_.neg_infin()) {
						// Prevent other write operations from being started.
						core_.pending_write_.expires_at(
						    core_.pos_infin());

						// Start writing all the data to the underlying transport.
						boost::asio::async_write(
						    next_layer_,
						    core_.engine_.get_output(
							core_.output_buffer_),
						    BOOST_ASIO_MOVE_CAST(io_op)(
							*this));
					} else {
			...
			default:
				...
				case engine::want_input_and_retry:
					...
					core_.input_ = boost::asio::buffer(
					    core_.input_buffer_,
					    bytes_transferred);
					core_.input_ = core_.engine_.put_input(
					    core_.input_);
			...
			op_.call_handler(handler_,
					 core_.engine_.map_error_code(ec_), 0);
		}
	}
