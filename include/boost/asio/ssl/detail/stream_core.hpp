...
struct stream_core {
	...
	stream_core(SSL_CTX *context, boost::asio::io_context &io_context)
	    : engine_(context), pending_read_(io_context),
	...
	static boost::asio::deadline_timer::time_type
	expiry(const boost::asio::deadline_timer &timer)
	{
	...
	// Helper function to get a timer's expiry time.
	static boost::asio::steady_timer::time_point
	expiry(const boost::asio::steady_timer &timer)
	{
		return timer.expiry();
	}
#endif // defined(BOOST_ASIO_HAS_BOOST_DATE_TIME)
