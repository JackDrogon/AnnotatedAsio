...
struct buffers_iterator_types {
	enum { is_mutable = is_convertible<typename BufferSequence::value_type,
...
public:
   ...
   */
	...
	buffers_iterator()
	    : current_buffer_(), current_buffer_position_(0), begin_(),
	      current_(), end_(), position_(0)
	{
	}
...
private:
	...
	{
		if (n > 0) {
			BOOST_ASIO_ASSERT(current_ != end_ &&
			...
			std::size_t abs_n = -n;
			BOOST_ASIO_ASSERT(position_ >= abs_n &&
					  "iterator out of bounds");
			for (;;) {
				// Check if the advance can be satisfied by the current buffer.
