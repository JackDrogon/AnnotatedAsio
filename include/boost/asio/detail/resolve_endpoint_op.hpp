...
public:
	...
	resolve_endpoint_op(socket_ops::weak_cancel_token_type cancel_token,
			    const endpoint_type &endpoint, io_context_impl &ioc,
			    Handler &handler)
	    : resolve_op(&resolve_endpoint_op::do_complete),
	      cancel_token_(cancel_token), endpoint_(endpoint),
				...
				std::size_t /*bytes_transferred*/)
	{
		...
		resolve_endpoint_op *o(
		    static_cast<resolve_endpoint_op *>(base));
		...
		if (owner && owner != &o->io_context_impl_) {
			...
			socket_ops::background_getnameinfo(
			    o->cancel_token_, o->endpoint_.data(),
			    o->endpoint_.size(), host_name, NI_MAXHOST,
			    service_name, NI_MAXSERV,
			    o->endpoint_.protocol().type(), o->ec_);
			o->results_ = results_type::create(
			...
			detail::binder2<Handler, boost::system::error_code,
					results_type>
