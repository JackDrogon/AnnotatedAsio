...
win_object_handle_service::win_object_handle_service(
    ...
    : service_base<win_object_handle_service>(io_context),
      ...
      mutex_(), impl_list_(0), shutdown_(false)
{
}
    ...
    win_object_handle_service::implementation_type &impl)
{
	...
	if (is_open(impl)) {
		BOOST_ASIO_HANDLER_OPERATION(
		    (io_context_.context(), "object_handle", &impl,
		     reinterpret_cast<uintmax_t>(impl.wait_handle_), "close"));
    ...
    boost::system::error_code &ec)
{
	if (is_open(impl)) {
		BOOST_ASIO_HANDLER_OPERATION(
		    (io_context_.context(), "object_handle", &impl,
		     reinterpret_cast<uintmax_t>(impl.wait_handle_), "cancel"));
