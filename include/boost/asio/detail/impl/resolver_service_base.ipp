...
resolver_service_base::resolver_service_base(
    ...
    : io_context_impl_(boost::asio::use_service<io_context_impl>(io_context)),
      ...
      work_io_context_impl_(
	  boost::asio::use_service<io_context_impl>(*work_io_context_)),
      work_(boost::asio::make_work_guard(*work_io_context_)), work_thread_(0)
{
}
    ...
    resolver_service_base::implementation_type &impl)
{
	BOOST_ASIO_HANDLER_OPERATION(
	    (io_context_impl_.context(), "resolver", &impl, 0, "cancel"));
...
void resolver_service_base::start_resolve_op(resolve_op *op)
{
	if (BOOST_ASIO_CONCURRENCY_HINT_IS_LOCKING(
		SCHEDULER, io_context_impl_.concurrency_hint())) {
		start_work_thread();
