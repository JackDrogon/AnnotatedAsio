    ...
    boost::asio::execution_context::fork_event fork_ev)
{
	if (fork_ev == boost::asio::execution_context::fork_child) {
		...
		for (descriptor_state *state = registered_descriptors_.first();
			...
			ev.data.ptr = state;
			int result = epoll_ctl(epoll_fd_, EPOLL_CTL_ADD,
...
void epoll_reactor::run(long usec, op_queue<operation> &ops)
{
	...
	for (int i = 0; i < num_events; ++i) {
		...
		if (ptr == &interrupter_) {
			...
			if (!ops.is_enqueued(descriptor_data)) {
				descriptor_data->set_ready_events(
				    events[i].events);
				ops.push(descriptor_data);
			} else {
				descriptor_data->add_ready_events(
				    events[i].events);
			}
		}
	}
	...
	if (check_timers) {
		...
		if (timer_fd_ != -1) {
			...
			timerfd_settime(timer_fd_, flags, &new_timeout,
					&old_timeout);
		}
#endif // defined(BOOST_ASIO_HAS_TIMERFD)
	}
...
void epoll_reactor::interrupt()
{
	...
	ev.events = EPOLLIN | EPOLLERR | EPOLLET;
	ev.data.ptr = &interrupter_;
	epoll_ctl(epoll_fd_, EPOLL_CTL_MOD, interrupter_.read_descriptor(),
		  &ev);
}
...
struct epoll_reactor::perform_io_cleanup_on_block_exit {
	...
	~perform_io_cleanup_on_block_exit()
	{
		if (first_op_) {
			...
			if (!ops_.empty())
				reactor_->scheduler_.post_deferred_completions(
				    ops_);
...
void epoll_reactor::descriptor_state::do_complete(
    void *owner, operation *base, const boost::system::error_code &ec,
    std::size_t bytes_transferred)
{
	if (owner) {
		descriptor_state *descriptor_data =
		    static_cast<descriptor_state *>(base);
