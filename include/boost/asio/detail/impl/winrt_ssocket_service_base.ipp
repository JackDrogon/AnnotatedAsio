...
winrt_ssocket_service_base::winrt_ssocket_service_base(
    ...
    : io_context_(use_service<io_context_impl>(io_context)),
      async_manager_(use_service<winrt_async_manager>(io_context)), mutex_(),
      impl_list_(0)
{
}
...
std::size_t winrt_ssocket_service_base::do_get_endpoint(
    ...
    std::size_t addr_len, boost::system::error_code &ec) const
{
	if (!is_open(impl)) {
	...
	try {
		std::string addr_string = winrt_utils::string(
			...
			: impl.socket_->Information->RemoteAddress
			      ->CanonicalName);
		unsigned short port = winrt_utils::integer(
		    local ? impl.socket_->Information->LocalPort
			  : impl.socket_->Information->RemotePort);
		unsigned long scope = 0;

		switch (reinterpret_cast<const socket_addr_type *>(addr)
			    ->sa_family) {
		case BOOST_ASIO_OS_DEF(AF_INET):
			if (addr_len < sizeof(sockaddr_in4_type)) {
				...
				socket_ops::inet_pton(
				    ...
				    addr_string.c_str(),
				    &reinterpret_cast<sockaddr_in4_type *>(addr)
					 ->sin_addr,
				    &scope, ec);
				reinterpret_cast<sockaddr_in4_type *>(addr)
				    ->sin_port =
		...
		case BOOST_ASIO_OS_DEF(AF_INET6):
			if (addr_len < sizeof(sockaddr_in6_type)) {
				...
				socket_ops::inet_pton(
				    ...
				    addr_string.c_str(),
				    &reinterpret_cast<sockaddr_in6_type *>(addr)
					 ->sin6_addr,
				    &scope, ec);
				reinterpret_cast<sockaddr_in6_type *>(addr)
				    ->sin6_port =
    ...
    boost::system::error_code &ec) const
{
	...
	try {
		if (level == BOOST_ASIO_OS_DEF(SOL_SOCKET) &&
			...
			if (*optlen >= sizeof(int)) {
				int value =
				    impl.socket_->Control->KeepAlive ? 1 : 0;
			...
			if (*optlen >= sizeof(int)) {
				int value =
				    impl.socket_->Control->NoDelay ? 1 : 0;
				std::memcpy(optval, &value, sizeof(int));
				*optlen = sizeof(int);
				ec = boost::system::error_code();
			} else {
    ...
    const void *addr, boost::system::error_code &ec)
{
	...
	case BOOST_ASIO_OS_DEF(AF_INET):
		socket_ops::inet_ntop(
		    BOOST_ASIO_OS_DEF(AF_INET),
		    &reinterpret_cast<const sockaddr_in4_type *>(addr)
			 ->sin_addr,
	...
	case BOOST_ASIO_OS_DEF(AF_INET6):
		socket_ops::inet_ntop(
		    BOOST_ASIO_OS_DEF(AF_INET6),
		    &reinterpret_cast<const sockaddr_in6_type *>(addr)
			 ->sin6_addr,
		    addr_string, sizeof(addr_string), 0, ec);
		port = socket_ops::network_to_host_short(
		    reinterpret_cast<const sockaddr_in6_type *>(addr)
    ...
    boost::system::error_code &ec)
{
	...
	try {
		...
		return async_manager_.sync(
		    impl.socket_->OutputStream->WriteAsync(bufs.buffers()[0]),
		    ec);
	} catch (Platform::Exception ^ e) {
		ec = boost::system::error_code(
		    e->HResult, boost::system::system_category());
    ...
    winrt_async_op<unsigned int> *op, bool is_continuation)
{
	...
	try {
		...
		if (bufs.all_empty()) {
			io_context_.post_immediate_completion(op,
							      is_continuation);
			return;
		}
