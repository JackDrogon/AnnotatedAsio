    ...
    boost::asio::execution_context::fork_event fork_ev)
{
	if (fork_ev == boost::asio::execution_context::fork_child) {
		...
		BOOST_ASIO_KQUEUE_EV_SET(
		    &events[0], interrupter_.read_descriptor(), EVFILT_READ,
		    EV_ADD, 0, 0, &interrupter_);
		if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1) {
			boost::system::error_code ec(
			    errno, boost::asio::error::get_system_category());
			boost::asio::detail::throw_error(
			    ec, "kqueue interrupter registration");
		}

		// Re-register all descriptors with kqueue.
		mutex::scoped_lock descriptors_lock(
		    registered_descriptors_mutex_);
		for (descriptor_state *state = registered_descriptors_.first();
		     state != 0; state = state->next_) {
			if (state->num_kevents_ > 0) {
				BOOST_ASIO_KQUEUE_EV_SET(
				    &events[0], state->descriptor_, EVFILT_READ,
				    EV_ADD | EV_CLEAR, 0, 0, state);
				BOOST_ASIO_KQUEUE_EV_SET(
				    &events[1], state->descriptor_,
				    EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0,
				    state);
				if (::kevent(kqueue_fd_, events,
					     state->num_kevents_, 0, 0,
					     0) == -1) {
					boost::system::error_code ec(
					    errno, boost::asio::error::
						       get_system_category());
					boost::asio::detail::throw_error(
					    ec, "kqueue re-registration");
				}
			}
		}
	}
    ...
    bool is_continuation, bool allow_speculative)
{
	...
	if (descriptor_data->op_queue_[op_type].empty()) {
		...
		if (allow_speculative &&
			...
			if (descriptor_data->num_kevents_ <
			    num_kevents[op_type]) {
				struct kevent events[2];
				BOOST_ASIO_KQUEUE_EV_SET(
				    &events[0], descriptor, EVFILT_READ,
				    EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
				BOOST_ASIO_KQUEUE_EV_SET(
				    &events[1], descriptor, EVFILT_WRITE,
				    EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
				if (::kevent(kqueue_fd_, events,
					     num_kevents[op_type], 0, 0,
					     0) != -1) {
					descriptor_data->num_kevents_ =
					    num_kevents[op_type];
				} else {
			...
			BOOST_ASIO_KQUEUE_EV_SET(
			    &events[1], descriptor, EVFILT_WRITE,
			    EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
			::kevent(kqueue_fd_, events,
				 descriptor_data->num_kevents_, 0, 0, 0);
		}
	}
    ...
    kqueue_reactor::per_descriptor_data &descriptor_data, bool closing)
{
	...
	if (!descriptor_data->shutdown_) {
		...
		for (int i = 0; i < max_ops; ++i) {
			while (reactor_op *op =
				   descriptor_data->op_queue_[i].front()) {
				op->ec_ = boost::asio::error::operation_aborted;
				descriptor_data->op_queue_[i].pop();
				ops.push(op);
			}
    ...
    kqueue_reactor::per_descriptor_data &descriptor_data)
{
	...
	if (!descriptor_data->shutdown_) {
		...
		BOOST_ASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
					 EV_DELETE, 0, 0, 0);
		::kevent(kqueue_fd_, events, descriptor_data->num_kevents_, 0,
			 0, 0);
...
void kqueue_reactor::run(long usec, op_queue<operation> &ops)
{
	...
	timespec *timeout =
	    usec ? get_timeout(usec, timeout_buf) : &timeout_buf;
	...
	for (int i = 0; i < num_events; ++i) {
		...
		if (ptr != &interrupter_) {
			...
			BOOST_ASIO_HANDLER_REACTOR_EVENTS(
			    (context(), reinterpret_cast<uintmax_t>(ptr),
			     event_mask));
		}
	}
	...
	for (int i = 0; i < num_events; ++i) {
		...
		if (ptr == &interrupter_) {
			...
			mutex::scoped_lock descriptor_lock(
			...
			if (events[i].filter == EVFILT_WRITE &&
				...
				BOOST_ASIO_KQUEUE_EV_SET(
				    &delete_events[0],
				    descriptor_data->descriptor_, EVFILT_WRITE,
				    EV_DELETE, 0, 0, 0);
