    ...
    const std::string &device, boost::system::error_code &ec)
{
	...
	if (s >= 0) {
		...
		::cfmakeraw(&ios);
#else
		ios.c_iflag &= ~(IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR |
				 IGNCR | ICRNL | IXON);
		ios.c_oflag &= ~OPOST;
		...
		errno = 0;
		s = descriptor_ops::error_wrapper(
		    ::tcsetattr(fd, TCSANOW, &ios), ec);
	}
	if (s < 0) {
		boost::system::error_code ignored_ec;
		descriptor_ops::close(fd, state, ignored_ec);
...
boost::system::error_code reactive_serial_port_service::do_set_option(
    reactive_serial_port_service::implementation_type &impl,
    reactive_serial_port_service::store_function_type store, const void *option,
    boost::system::error_code &ec)
{
	termios ios;
	errno = 0;
	descriptor_ops::error_wrapper(
	    ::tcgetattr(descriptor_service_.native_handle(impl), &ios), ec);
	if (ec)
		return ec;
	...
	errno = 0;
	descriptor_ops::error_wrapper(
	    ::tcsetattr(descriptor_service_.native_handle(impl), TCSANOW, &ios),
	    ec);
	return ec;
}
...
boost::system::error_code reactive_serial_port_service::do_get_option(
    ...
    reactive_serial_port_service::load_function_type load, void *option,
