...
public:
	...
	invoker(invoker &&other)
	    : impl_(BOOST_ASIO_MOVE_CAST(implementation_type)(other.impl_)),
	      work_(BOOST_ASIO_MOVE_CAST(executor_work_guard<Executor>)(
		  other.work_))
	{
	}
	...
	struct on_invoker_exit {
		...
		~on_invoker_exit()
		{
			...
			this_->impl_->ready_queue_.push(
			    this_->impl_->waiting_queue_);
			...
			if (more_handlers) {
				...
				ex.post(BOOST_ASIO_MOVE_CAST(invoker)(*this_),
					allocator);
			}
		}
	};
