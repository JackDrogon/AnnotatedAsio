...
struct win_iocp_io_context::work_finished_on_block_exit {
	~work_finished_on_block_exit() { io_context_->work_finished(); }
...
struct win_iocp_io_context::timer_thread_function {
	void operator()()
	{
		while (::InterlockedExchangeAdd(&io_context_->shutdown_, 0) ==
		       0) {
			if (::WaitForSingleObject(
				io_context_->waitable_timer_.handle,
				INFINITE) == WAIT_OBJECT_0) {
				::InterlockedExchange(
				    &io_context_->dispatch_required_, 1);
				::PostQueuedCompletionStatus(
				    io_context_->iocp_.handle, 0,
				    wake_for_dispatch, 0);
			}
		}
	}
...
win_iocp_io_context::win_iocp_io_context(boost::asio::execution_context &ctx,
      ...
      concurrency_hint_(concurrency_hint)
{
	...
	iocp_.handle = ::CreateIoCompletionPort(
	    ...
	    static_cast<DWORD>(concurrency_hint >= 0 ? concurrency_hint
						     : DWORD(~0)));
	if (!iocp_.handle) {
...
void win_iocp_io_context::shutdown()
{
	...
	if (timer_thread_.get()) {
		...
		timeout.QuadPart = 1;
		::SetWaitableTimer(waitable_timer_.handle, &timeout, 1, 0, 0,
				   FALSE);
	}
	...
	while (::InterlockedExchangeAdd(&outstanding_work_, 0) > 0) {
		...
		if (!ops.empty()) {
			...
			::GetQueuedCompletionStatus(
			    iocp_.handle, &bytes_transferred, &completion_key,
			    &overlapped, gqcs_timeout_);
			if (overlapped) {
				::InterlockedDecrement(&outstanding_work_);
...
win_iocp_io_context::register_handle(HANDLE handle,
				     boost::system::error_code &ec)
{
...
void win_iocp_io_context::stop()
{
	if (::InterlockedExchange(&stopped_, 1) == 0) {
		if (::InterlockedExchange(&stop_event_posted_, 1) == 0) {
			if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0,
							  0)) {
				DWORD last_error = ::GetLastError();
				boost::system::error_code ec(
				    last_error,
...
void win_iocp_io_context::abandon_operations(op_queue<win_iocp_operation> &ops)
{
	while (win_iocp_operation *op = ops.front()) {
		ops.pop();
		::InterlockedDecrement(&outstanding_work_);
...
void win_iocp_io_context::on_pending(win_iocp_operation *op)
{
	if (::InterlockedCompareExchange(&op->ready_, 1, 0) == 1) {
		...
		if (!::PostQueuedCompletionStatus(
			iocp_.handle, 0, overlapped_contains_result, op)) {
			// Out of resources. Put on completed queue instead.
			mutex::scoped_lock lock(dispatch_mutex_);
			completed_ops_.push(op);
			::InterlockedExchange(&dispatch_required_, 1);
...
size_t win_iocp_io_context::do_one(DWORD msec, boost::system::error_code &ec)
{
	for (;;) {
		...
		if (::InterlockedCompareExchange(&dispatch_required_, 0, 1) ==
		    1) {
			mutex::scoped_lock lock(dispatch_mutex_);
		...
		BOOL ok = ::GetQueuedCompletionStatus(
		    iocp_.handle, &bytes_transferred, &completion_key,
		    &overlapped, msec < gqcs_timeout_ ? msec : gqcs_timeout_);
		DWORD last_error = ::GetLastError();
		...
		if (overlapped) {
			...
			if (completion_key == overlapped_contains_result) {
				result_ec = boost::system::error_code(
			...
			if (::InterlockedCompareExchange(&op->ready_, 1, 0) ==
			    1) {
				// Ensure the count of outstanding work is decremented on block exit.
				work_finished_on_block_exit on_exit = {this};
				(void)on_exit;

				op->complete(this, result_ec,
					     bytes_transferred);
				ec = boost::system::error_code();
				return 1;
			}
			...
			if (::InterlockedExchangeAdd(&stopped_, 0) != 0) {
				...
				if (::InterlockedExchange(&stop_event_posted_,
							  1) == 0) {
					if (!::PostQueuedCompletionStatus(
						iocp_.handle, 0, 0, 0)) {
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base &queue)
{
	...
	if (!waitable_timer_.handle) {
		...
		timeout.QuadPart *= 10;
		::SetWaitableTimer(waitable_timer_.handle, &timeout,
				   max_timeout_msec, 0, 0, FALSE);
	}
