...
void boost_asio_signal_handler(int signal_number)
{
	...
	signed_size_type result = ::write(
	    state->write_descriptor_, &signal_number, sizeof(signal_number));
	(void)result;
	errno = saved_errno;
...
public:
	...
	static status do_perform(reactor_op *)
	{
		...
		while (::read(fd, &signal_number, sizeof(int)) == sizeof(int))
			if (signal_number >= 0 &&
			    signal_number < max_signal_number)
				signal_set_service::deliver_signal(
				    signal_number);
...
signal_set_service::signal_set_service(boost::asio::io_context &io_context)
    ...
    boost::asio::io_context::fork_event fork_ev)
{
	...
	case boost::asio::io_context::fork_prepare: {
		...
		lock.unlock();
		reactor_.deregister_internal_descriptor(read_descriptor,
							reactor_data_);
	...
	case boost::asio::io_context::fork_parent:
		if (state->fork_prepared_) {
			...
			reactor_.register_internal_descriptor(
			    reactor::read_op, read_descriptor, reactor_data_,
			    new pipe_read_op);
		}
		break;
			...
			int signal_number, boost::system::error_code &ec)
{
	...
	if (next == 0 || next->signal_number_ != signal_number) {
		...
		if (state->registration_count_[signal_number] == 0) {
			...
			if (::signal(signal_number,
				     boost_asio_signal_handler) == SIG_ERR)
		...
		*insertion_point = new_registration;
		...
		new_registration->next_in_table_ =
		    registrations_[signal_number];
		if (registrations_[signal_number])
			registrations_[signal_number]->prev_in_table_ =
			    new_registration;
	...
	if (reg != 0 && reg->signal_number_ == signal_number) {
		...
		*deletion_point = reg->next_in_set_;
		...
		if (reg->prev_in_table_)
			reg->prev_in_table_->next_in_table_ =
			    reg->next_in_table_;
		if (reg->next_in_table_)
			reg->next_in_table_->prev_in_table_ =
			    reg->prev_in_table_;
			  ...
			  boost::system::error_code &ec)
{
	...
	while (registration *reg = impl.signals_) {
		...
		if (registrations_[reg->signal_number_] == reg)
			registrations_[reg->signal_number_] =
			    reg->next_in_table_;
		if (reg->prev_in_table_)
			reg->prev_in_table_->next_in_table_ =
			    reg->next_in_table_;
			   ...
			   boost::system::error_code &ec)
{
	BOOST_ASIO_HANDLER_OPERATION(
	    (io_context_.context(), "signal_set", &impl, 0, "cancel"));
...
void signal_set_service::add_service(signal_set_service *service)
{
	...
	if (state->service_list_ != 0) {
		if (!BOOST_ASIO_CONCURRENCY_HINT_IS_LOCKING(
			SCHEDULER, service->io_context_.concurrency_hint()) ||
...
void signal_set_service::remove_service(signal_set_service *service)
{
	...
	if (service->next_ || service->prev_ ||
	    state->service_list_ == service) {
#if !defined(BOOST_ASIO_WINDOWS) && !defined(BOOST_ASIO_WINDOWS_RUNTIME) &&    \
    !defined(__CYGWIN__)
		...
		service->reactor_.cleanup_descriptor_data(
