...
inline socket_type call_accept(SockLenType msghdr::*, socket_type s,
...
socket_type accept(socket_type s, socket_addr_type *addr, std::size_t *addrlen,
		   boost::system::error_code &ec)
{
	if (s == invalid_socket) {
	...
	socket_type new_s = error_wrapper(
	    call_accept(&msghdr::msg_namelen, s, addr, addrlen), ec);
	if (new_s == invalid_socket)
		return new_s;

#if defined(__MACH__) && defined(__APPLE__) || defined(__FreeBSD__)
	int optval = 1;
	int result = error_wrapper(::setsockopt(new_s, SOL_SOCKET, SO_NOSIGPIPE,
...
socket_type sync_accept(socket_type s, state_type state, socket_addr_type *addr,
			std::size_t *addrlen, boost::system::error_code &ec)
{
	...
	for (;;) {
		...
		socket_type new_socket =
		    socket_ops::accept(s, addr, addrlen, ec);
...
void complete_iocp_accept(socket_type s, void *output_buffer,
			  DWORD address_length, socket_addr_type *addr,
	...
	if (!ec) {
		...
		if (addr && addrlen) {
			...
			GetAcceptExSockaddrs(output_buffer, 0, address_length,
					     address_length, &local_addr,
					     &local_addr_length, &remote_addr,
					     &remote_addr_length);
			if (static_cast<std::size_t>(remote_addr_length) >
				...
				*addrlen = static_cast<std::size_t>(
				    remote_addr_length);
			}
		}
...
bool non_blocking_accept(socket_type s, state_type state,
			 socket_addr_type *addr, std::size_t *addrlen,
...
template <typename SockLenType>
inline int call_bind(SockLenType msghdr::*, socket_type s,
		     const socket_addr_type *addr, std::size_t addrlen)
{
	return ::bind(s, addr, (SockLenType)addrlen);
}

int bind(socket_type s, const socket_addr_type *addr, std::size_t addrlen,
	 boost::system::error_code &ec)
{
	if (s == invalid_socket) {
		ec = boost::asio::error::bad_descriptor;
	...
	int result = error_wrapper(
	    call_bind(&msghdr::msg_namelen, s, addr, addrlen), ec);
	if (result == 0)
		ec = boost::system::error_code();
	return result;
}
	  ...
	  boost::system::error_code &ec)
{
	...
	if (s != invalid_socket) {
		...
		if (destruction && (state & user_set_linger)) {
			...
			socket_ops::setsockopt(s, state, SOL_SOCKET, SO_LINGER,
		...
		if (result != 0 && (ec == boost::asio::error::would_block ||
				    ec == boost::asio::error::try_again)) {
			   ...
			   boost::system::error_code &ec)
{
	...
	if (result >= 0) {
		clear_last_error();
		int flag =
		    (value ? (result | O_NONBLOCK) : (result & ~O_NONBLOCK));
	...
	if (result >= 0) {
		...
		if (value)
			...
			state &=
			    ~(user_set_non_blocking | internal_non_blocking);
		}
		return true;
	}
...
template <typename SockLenType>
inline int call_connect(SockLenType msghdr::*, socket_type s,
			const socket_addr_type *addr, std::size_t addrlen)
{
	return ::connect(s, addr, (SockLenType)addrlen);
}

int connect(socket_type s, const socket_addr_type *addr, std::size_t addrlen,
	    boost::system::error_code &ec)
{
	if (s == invalid_socket) {
		ec = boost::asio::error::bad_descriptor;
	...
	clear_last_error();
	int result = error_wrapper(
	    call_connect(&msghdr::msg_namelen, s, addr, addrlen), ec);
	if (result == 0)
		ec = boost::system::error_code();
		  ...
		  std::size_t addrlen, boost::system::error_code &ec)
{
	...
	ec = boost::system::error_code(
	    connect_error, boost::asio::error::get_system_category());
}
		      ...
		      boost::system::error_code &ec)
{
	...
	int result =
	    error_wrapper(::WSARecv(s, bufs, recv_buf_count, &bytes_transferred,
				    &recv_flags, 0, 0),
			  ec);
	if (ec.value() == ERROR_NETNAME_DELETED)
		 ...
		 int flags, bool all_empty, boost::system::error_code &ec)
{
	...
	for (;;) {
		...
		signed_size_type bytes =
		    socket_ops::recv(s, bufs, count, flags, ec);
			...
			size_t bytes_transferred)
{
	...
	// Check for connection closed.
		 ...
		 (state & stream_oriented) != 0 && !all_empty) {
		ec = boost::asio::error::eof;
	}
}
			  ...
			  boost::system::error_code &ec)
{
	...
	int result = error_wrapper(
	    ::WSARecvFrom(s, bufs, recv_buf_count, &bytes_transferred,
...
signed_size_type recvmsg(socket_type s, buf *bufs, size_t count, int in_flags,
			 int &out_flags, boost::system::error_code &ec)
{
	clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
	out_flags = 0;
	...
	signed_size_type result =
	    error_wrapper(::recvmsg(s, &msg, in_flags), ec);
	if (result >= 0) {
		ec = boost::system::error_code();
		      ...
		      boost::system::error_code &ec)
{
	...
	int result =
	    error_wrapper(::WSASend(s, const_cast<buf *>(bufs), send_buf_count,
		 ...
		 int flags, bool all_empty, boost::system::error_code &ec)
{
	...
	for (;;) {
		// Try to complete the operation without blocking.
		signed_size_type bytes =
		    socket_ops::send(s, bufs, count, flags, ec);
			...
			boost::system::error_code &ec)
{
	...
	DWORD bytes_transferred = 0;
	int result =
	    error_wrapper(::WSASendTo(s, const_cast<buf *>(bufs),
		   ...
		   boost::system::error_code &ec)
{
	clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
	socket_type s = error_wrapper(
	    ::WSASocketW(af, type, protocol, 0, 0, WSA_FLAG_OVERLAPPED), ec);
	if (s == invalid_socket)
		return s;
	       ...
	       boost::system::error_code &ec)
{
	...
	if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
		typedef int(WSAAPI * sso_t)(SOCKET, int, int, const char *,
					    int);
		if (sso_t sso =
			(sso_t)::GetProcAddress(winsock_module, "setsockopt")) {
			clear_last_error();
	...
	clear_last_error();
	int result =
	    error_wrapper(call_setsockopt(&msghdr::msg_namelen, s, level,
    ...
    defined(__FreeBSD__) || defined(__OpenBSD__)
		...
		if ((state & datagram_oriented) && level == SOL_SOCKET &&
			...
			call_setsockopt(&msghdr::msg_namelen, s, SOL_SOCKET,
			   ...
			   int optname, void *optval, std::size_t *optlen)
{
	...
	int result =
	    ::getsockopt(s, level, optname, (char *)optval, &tmp_optlen);
	       ...
	       void *optval, size_t *optlen, boost::system::error_code &ec)
{
	...
	if (level == custom_socket_option_level &&
		...
		*static_cast<int *>(optval) =
		    (state & enable_connection_aborted) ? 1 : 0;
		ec = boost::system::error_code();
		return 0;
	}
	...
	if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
		...
		if (gso_t gso =
			(gso_t)::GetProcAddress(winsock_module, "getsockopt")) {
			clear_last_error();
			...
			*optlen = static_cast<size_t>(tmp_optlen);
			if (result != 0 && level == IPPROTO_IPV6 &&
			    optname == IPV6_V6ONLY &&
			    ec.value() == WSAENOPROTOOPT &&
			    *optlen == sizeof(DWORD)) {
	...
	clear_last_error();
	int result =
	    error_wrapper(call_getsockopt(&msghdr::msg_namelen, s, level,
					  optname, optval, optlen),
			  ec);
...
template <typename SockLenType>
inline int call_getpeername(SockLenType msghdr::*, socket_type s,
			    socket_addr_type *addr, std::size_t *addrlen)
{
...
int getpeername(socket_type s, socket_addr_type *addr, std::size_t *addrlen,
		bool cached, boost::system::error_code &ec)
{
	...
	clear_last_error();
	int result = error_wrapper(
	    call_getpeername(&msghdr::msg_namelen, s, addr, addrlen), ec);
	if (result == 0)
		ec = boost::system::error_code();
	return result;
}
...
template <typename SockLenType>
inline int call_getsockname(SockLenType msghdr::*, socket_type s,
			    socket_addr_type *addr, std::size_t *addrlen)
{
	SockLenType tmp_addrlen = (SockLenType)*addrlen;
...
int getsockname(socket_type s, socket_addr_type *addr, std::size_t *addrlen,
		boost::system::error_code &ec)
{
	if (s == invalid_socket) {
		ec = boost::asio::error::bad_descriptor;
	...
	clear_last_error();
	int result = error_wrapper(
	    call_getsockname(&msghdr::msg_namelen, s, addr, addrlen), ec);
	if (result == 0)
		ec = boost::system::error_code();
	return result;
}
	  ...
	  boost::system::error_code &ec)
{
    ...
    defined(__FreeBSD__) || defined(__OpenBSD__)
	int result =
	    error_wrapper(::ioctl(s, static_cast<unsigned int>(cmd), arg), ec);
#else
...
int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
	   timeval *timeout, boost::system::error_code &ec)
{
	clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
...
#else
	int result = error_wrapper(
	    ::select(nfds, readfds, writefds, exceptfds, timeout), ec);
	if (result >= 0)
		ec = boost::system::error_code();
	return result;
#endif
...
int poll_read(socket_type s, state_type state, int msec,
	      boost::system::error_code &ec)
{
	if (s == invalid_socket) {
		ec = boost::asio::error::bad_descriptor;
...
int poll_connect(socket_type s, int msec, boost::system::error_code &ec)
{
	...
	clear_last_error();
	int result = error_wrapper(
	    ::select(s + 1, 0, &write_fds, &except_fds, timeout), ec);
	if (result >= 0)
		ec = boost::system::error_code();
	return result;
#else  // defined(BOOST_ASIO_WINDOWS)                                          \
		      ...
		      unsigned long scope_id, boost::system::error_code &ec)
{
	...
	if (af == BOOST_ASIO_OS_DEF(AF_INET)) {
		sprintf_s(dest, length, "%u.%u.%u.%u", bytes[0], bytes[1],
			  bytes[2], bytes[3]);
		return dest;
		...
		while (n < length && b < 16) {
			if (bytes[b] == 0 && bytes[b + 1] == 0 && z == 0) {
				do
					b += 2;
				while (b < 16 && bytes[b] == 0 &&
				       bytes[b + 1] == 0);
				n += sprintf_s(dest + n, length - n, ":%s",
					       b < 16 ? "" : ":"),
				    ++z;
			} else {
				n += sprintf_s(
				    dest + n, length - n, "%s%x", b ? ":" : "",
				    (static_cast<u_long_type>(bytes[b]) << 8) |
					bytes[b + 1]);
				b += 2;
			}
		}
	...
	int result = error_wrapper(
	    ::WSAAddressToStringW(&address.base, address_length, 0,
...
#else
	int result =
	    error_wrapper(::WSAAddressToStringA(&address.base, address_length,
						0, dest, &string_length),
			  ec);
#endif
...
#else  // defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
	const char *result = error_wrapper(
	    ::inet_ntop(af, src, dest, static_cast<int>(length)), ec);
	if (result == 0 && !ec)
		ec = boost::asio::error::invalid_argument;
	if (result != 0 && af == BOOST_ASIO_OS_DEF(AF_INET6) && scope_id != 0) {
		using namespace std; // For strcat and sprintf.
		char if_name[IF_NAMESIZE + 1] = "%";
		const in6_addr_type *ipv6_address =
		    static_cast<const in6_addr_type *>(src);
		bool is_link_local =
		    ((ipv6_address->s6_addr[0] == 0xfe) &&
		     ((ipv6_address->s6_addr[1] & 0xc0) == 0x80));
		bool is_multicast_link_local =
		    ((ipv6_address->s6_addr[0] == 0xff) &&
		     ((ipv6_address->s6_addr[1] & 0x0f) == 0x02));
		if ((!is_link_local && !is_multicast_link_local) ||
		    if_indextoname(static_cast<unsigned>(scope_id),
				   if_name + 1) == 0)
	      ...
	      boost::system::error_code &ec)
{
		...
		while (state != done) {
			...
			case bword:
				if (*p >= '0' && *p <= '9')
					...
					if (num_front_bytes + num_back_bytes ==
					    16) {
						ec = boost::asio::error::
						    invalid_argument;
						return -1;
	...
	int result =
	    error_wrapper(::WSAStringToAddressW(wide_buffer, af, 0,
						&address.base, &address_length),
			  ec);
#else
	int result =
	    error_wrapper(::WSAStringToAddressA(const_cast<char *>(src), af, 0,
						&address.base, &address_length),
			  ec);
#endif
	...
	if (af == BOOST_ASIO_OS_DEF(AF_INET)) {
		if (result != socket_error_retval) {
			memcpy(dest, &address.v4.sin_addr,
			       sizeof(in4_addr_type));
			ec = boost::system::error_code();
		} else if (strcmp(src, "255.255.255.255") == 0) {
			static_cast<in4_addr_type *>(dest)->s_addr =
			    INADDR_NONE;
			ec = boost::system::error_code();
		}
	} else // AF_INET6
	{
		if (result != socket_error_retval) {
			memcpy(dest, &address.v6.sin6_addr,
	...
	if (result > 0 && is_v6 && scope_id) {
		...
		*scope_id = 0;
		if (if_name != 0) {
			in6_addr_type *ipv6_address =
			    static_cast<in6_addr_type *>(dest);
			bool is_link_local =
			    ((ipv6_address->s6_addr[0] == 0xfe) &&
			     ((ipv6_address->s6_addr[1] & 0xc0) == 0x80));
			bool is_multicast_link_local =
		...
		for (unsigned i = 0; i < hostnames->Size; ++i) {
			...
			if (hostname->Type == HostNameType::DomainName) {
				std::wstring_convert<std::codecvt_utf8<wchar_t>>
				    converter;
				std::string raw_name = converter.to_bytes(
				    hostname->RawName->Data());
				if (namelen > 0 &&
				    raw_name.size() <
					static_cast<std::size_t>(namelen)) {
					strcpy_s(name, namelen,
						 raw_name.c_str());
					return 0;
				}
			}
			      ...
			      boost::system::error_code &ec)
{
	...
	*result = *retval;
	...
	int error = 0;
	hostent *retval =
	    error_wrapper(::getipnodebyaddr(addr, length, af, &error), ec);
	if (error)
		ec = translate_netdb_error(error);
	if (!retval)
		return 0;
			      ...
			      boost::system::error_code &ec)
{
	...
	*result = *retval;
	...
	int error = 0;
	hostent *retval = error_wrapper(
	    ::gethostbyname_r(name, result, buffer, buflength, &error), ec);
	if (error)
		ec = translate_netdb_error(error);
	return retval;
#elif defined(__MACH__) && defined(__APPLE__)
	...
	int error = 0;
	hostent *retval =
	    error_wrapper(::getipnodebyname(name, af, ai_flags, &error), ec);
	if (error)
		ec = translate_netdb_error(error);
	if (!retval)
		return 0;
		    ...
		    const char *serv)
{
	...
	if (
		...
		if (hints->ai_socktype == 0 ||
			...
			if (sptr != 0) {
				int rc =
				    gai_port(aihead, sptr->s_port, SOCK_STREAM);
				if (rc < 0)
					return EAI_MEMORY;
				num_found += rc;
			}
		...
		if (hints->ai_socktype == 0 ||
			...
			if (sptr != 0) {
				int rc =
				    gai_port(aihead, sptr->s_port, SOCK_DGRAM);
				if (rc < 0)
					return EAI_MEMORY;
				num_found += rc;
			}
				 ...
				 addrinfo_type **result)
{
	...
	for (gai_search *sptr = search; sptr < search + search_count; ++sptr) {
		...
		if (socket_ops::inet_pton(BOOST_ASIO_OS_DEF(AF_INET),
			...
			if (sptr->family == BOOST_ASIO_OS_DEF(AF_INET)) {
				rc = gai_aistruct(&ainext, &hints, &inaddr,
						  BOOST_ASIO_OS_DEF(AF_INET));
				if (rc != 0) {
					freeaddrinfo_emulation(aihead);
					gai_free(canon);
		...
		hostent hent;
		char hbuf[8192] = "";
		hostent *hptr = socket_ops::gethostbyname(
		    sptr->host, sptr->family, &hent, hbuf, sizeof(hbuf),
		    hints.ai_flags, ec);
		if (hptr == 0) {
			if (search_count == 2) {
		...
		if (host != 0 && host[0] != '\0' && hptr->h_name &&
		    hptr->h_name[0] && (hints.ai_flags & AI_CANONNAME) &&
		    canon == 0) {
		...
		for (char **ap = hptr->h_addr_list; *ap; ++ap) {
			rc = gai_aistruct(&ainext, &hints, *ap,
					  hptr->h_addrtype);
			if (rc != 0) {
				freeaddrinfo_emulation(aihead);
				gai_free(canon);
	...
	if (host != 0 && host[0] != '\0' && (hints.ai_flags & AI_CANONNAME)) {
		if (canon) {
			...
			gai_strcpy(aihead->ai_canonname, search[0].host,
				   canonname_len);
		}
	}
	gai_free(canon);
...
getnameinfo_emulation(const socket_addr_type *sa, std::size_t salen, char *host,
		      ...
		      int flags, boost::system::error_code &ec)
{
	...
	case BOOST_ASIO_OS_DEF(AF_INET):
		...
		port =
		    reinterpret_cast<const sockaddr_in4_type *>(sa)->sin_port;
		break;
	case BOOST_ASIO_OS_DEF(AF_INET6):
		...
		addr = reinterpret_cast<const char *>(
		    &reinterpret_cast<const sockaddr_in6_type *>(sa)
			 ->sin6_addr);
		addr_len = sizeof(in6_addr_type);
		port =
		    reinterpret_cast<const sockaddr_in6_type *>(sa)->sin6_port;
		break;
	default:
	...
	if (host && hostlen > 0) {
		if (flags & NI_NUMERICHOST) {
			if (socket_ops::inet_ntop(sa->sa_family, addr, host,
						  hostlen, 0, ec) == 0) {
				return ec;
			}
		} else {
			...
			hostent *hptr = socket_ops::gethostbyaddr(
			    addr, static_cast<int>(addr_len), sa->sa_family,
			...
			if (hptr && hptr->h_name && hptr->h_name[0] != '\0') {
				...
				if (flags & NI_NAMEREQD) {
					return ec = boost::asio::error::
						   host_not_found;
				}
				if (socket_ops::inet_ntop(sa->sa_family, addr,
							  host, hostlen, 0,
							  ec) == 0) {
					return ec;
				}
...
#endif // defined(BOOST_ASIO_HAS_PTHREADS)
			servent *sptr = ::getservbyport(
			    port, (flags & NI_DGRAM) ? "udp" : 0);
...
boost::system::error_code getaddrinfo(const char *host, const char *service,
				      ...
				      boost::system::error_code &ec)
{
	...
	if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
		if (gai_t gai = (gai_t)::GetProcAddress(winsock_module,
							"getaddrinfo")) {
	...
	if (error == 0 && service &&
		...
		for (addrinfo_type *ai = *result; ai; ai = ai->ai_next) {
			...
			case BOOST_ASIO_OS_DEF(AF_INET): {
				sockaddr_in4_type *sinptr =
				    reinterpret_cast<sockaddr_in4_type *>(
					ai->ai_addr);
			...
			case BOOST_ASIO_OS_DEF(AF_INET6): {
				sockaddr_in6_type *sin6ptr =
				    reinterpret_cast<sockaddr_in6_type *>(
					ai->ai_addr);
...
void freeaddrinfo(addrinfo_type *ai)
{
	...
	if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
		if (fai_t fai = (fai_t)::GetProcAddress(winsock_module,
							"freeaddrinfo")) {
			fai(ai);
			return;
		}
				      ...
				      boost::system::error_code &ec)
{
	...
	// Building for Windows XP, Windows Server 2003, or later.
	clear_last_error();
	int error = ::getnameinfo(addr, static_cast<socklen_t>(addrlen), host,
				  static_cast<DWORD>(hostlen), serv,
				  static_cast<DWORD>(servlen), flags);
	return ec = translate_addrinfo_error(error);
#else
	// Building for Windows 2000 or earlier.
	typedef int(WSAAPI * gni_t)(const socket_addr_type *, int, char *,
				    DWORD, char *, DWORD, int);
	if (HMODULE winsock_module = ::GetModuleHandleA("ws2_32")) {
		if (gni_t gni = (gni_t)::GetProcAddress(winsock_module,
							"getnameinfo")) {
			clear_last_error();
			int error = gni(addr, static_cast<int>(addrlen), host,
					static_cast<DWORD>(hostlen), serv,
					static_cast<DWORD>(servlen), flags);
			return ec = translate_addrinfo_error(error);
		}
	...
	clear_last_error();
	return getnameinfo_emulation(addr, addrlen, host, hostlen, serv,
				     servlen, flags, ec);
#endif
#elif !defined(BOOST_ASIO_HAS_GETADDRINFO)
	using namespace std; // For memcpy.
...
boost::system::error_code sync_getnameinfo(const socket_addr_type *addr,
					   std::size_t addrlen, char *host,
					   std::size_t hostlen, char *serv,
					   ...
					   boost::system::error_code &ec)
{
	...
	socket_ops::getnameinfo(addr, addrlen, host, hostlen, serv, servlen,
				flags, ec);
	if (ec) {
		socket_ops::getnameinfo(addr, addrlen, host, hostlen, serv,
					servlen, flags | NI_NUMERICSERV, ec);
	}
