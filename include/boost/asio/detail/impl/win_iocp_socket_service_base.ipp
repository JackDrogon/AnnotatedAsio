...
win_iocp_socket_service_base::win_iocp_socket_service_base(
    ...
    : io_context_(io_context),
      iocp_service_(use_service<win_iocp_io_context>(io_context)), reactor_(0),
...
#if defined(BOOST_ASIO_ENABLE_CANCELIO)
	impl.safe_cancellation_thread_id_ =
	    other_impl.safe_cancellation_thread_id_;
...
{
	if (is_open(impl)) {
		BOOST_ASIO_HANDLER_OPERATION((iocp_service_.context(), "socket",
					      &impl, impl.socket_, "close"));
		...
		if (r)
			r->deregister_descriptor(impl.socket_,
    ...
    boost::system::error_code &ec)
{
	...
	if (FARPROC cancel_io_ex_ptr = ::GetProcAddress(
		...
		ec = boost::system::error_code();
	} else if (impl.safe_cancellation_thread_id_ ==
		   ::GetCurrentThreadId()) {
    ...
    int type, int protocol, boost::system::error_code &ec)
{
	...
	impl.cancel_token_.reset(static_cast<void *>(0),
				 socket_ops::noop_deleter());
	ec = boost::system::error_code();
	return ec;
}
...
void win_iocp_socket_service_base::start_send_to_op(
    win_iocp_socket_service_base::base_implementation_type &impl,
    WSABUF *buffers, std::size_t buffer_count, const socket_addr_type *addr,
    ...
    int protocol, void *output_buffer, DWORD address_length, operation *op)
{
	...
	if (!is_open(impl))
		...
		boost::system::error_code ec;
		new_socket.reset(
		    socket_ops::socket(family, type, protocol, ec));
		if (new_socket.get() == invalid_socket)
			iocp_service_.on_completion(op, ec);
		else {
			DWORD bytes_read = 0;
			BOOL result = ::AcceptEx(
			    impl.socket_, new_socket.get(), output_buffer, 0,
    ...
    int protocol, void *output_buffer, DWORD address_length, operation *op)
{
	...
	if (new_socket.get() == invalid_socket)
		...
		DWORD bytes_read = 0;
		BOOL result =
		    ::AcceptEx(s, new_socket.get(), output_buffer, 0,
			       address_length, address_length, &bytes_read, op);
		DWORD last_error = ::WSAGetLastError();
		if (!result && last_error != WSA_IO_PENDING)
			iocp_service_.on_completion(op, last_error);
...
void win_iocp_socket_service_base::start_connect_op(
    win_iocp_socket_service_base::base_implementation_type &impl, int family,
    int type, const socket_addr_type *addr, std::size_t addrlen,
    win_iocp_socket_connect_op_base *op)
{
	...
	if ((impl.state_ & socket_ops::non_blocking) != 0 ||
	    socket_ops::set_internal_non_blocking(impl.socket_, impl.state_,
						  true, op->ec_)) {
		if (socket_ops::connect(impl.socket_, addr, addrlen, op->ec_) !=
		    0) {
			if (op->ec_ == boost::asio::error::in_progress ||
			    op->ec_ == boost::asio::error::would_block) {
				op->ec_ = boost::system::error_code();
				r.start_op(select_reactor::connect_op,
					   impl.socket_, impl.reactor_data_, op,
					   false, false);
				return;
			}
		}
...
select_reactor &win_iocp_socket_service_base::get_reactor()
{
	...
	if (!r) {
		...
		interlocked_exchange_pointer(
		    reinterpret_cast<void **>(&reactor_), r);
	}
	return *r;
}
    ...
    win_iocp_socket_service_base::base_implementation_type &impl, int type)
{
	...
	if (!ptr) {
		...
		if (::WSAIoctl(impl.socket_, SIO_GET_EXTENSION_FUNCTION_POINTER,
			       &guid, sizeof(guid), &ptr, sizeof(ptr), &bytes,
			       0, 0) != 0) {
