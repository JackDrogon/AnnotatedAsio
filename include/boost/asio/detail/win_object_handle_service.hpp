...
class win_object_handle_service : public service_base<win_object_handle_service>
{
public:
	...
	public:
		...
		implementation_type()
		    : handle_(INVALID_HANDLE_VALUE),
		      wait_handle_(INVALID_HANDLE_VALUE), owner_(0), next_(0),
		      prev_(0)
		{
		}
	...
	// Constructor.
	BOOST_ASIO_DECL
	win_object_handle_service(boost::asio::io_context &io_context);
	...
	void async_wait(implementation_type &impl, Handler &handler)
	{
		...
		BOOST_ASIO_HANDLER_CREATION(
		    (io_context_.context(), *p.p, "object_handle", &impl,
...
private:
	...
	BOOST_ASIO_DECL void start_wait_op(implementation_type &impl,
					   wait_op *op);

	// Helper function to register a wait operation.
	BOOST_ASIO_DECL void register_wait_callback(implementation_type &impl,
						    mutex::scoped_lock &lock);

	// Callback function invoked when the registered wait completes.
	static BOOST_ASIO_DECL VOID CALLBACK wait_callback(PVOID param,
							   BOOLEAN timeout);
