...
public:
	...
	public:
		...
		native_handle_type(socket_type s, const endpoint_type &ep)
		    : socket_(s), have_remote_endpoint_(true),
		      remote_endpoint_(ep)
		{
		}
	...
	struct implementation_type
		...
		implementation_type()
		    : protocol_(endpoint_type().protocol()),
		      have_remote_endpoint_(false), remote_endpoint_()
		{
		}
	...
	assign(implementation_type &impl, const protocol_type &protocol,
	       ...
	       boost::system::error_code &ec)
	{
		if (!do_assign(impl, protocol.type(), native_socket, ec)) {
			...
			impl.have_remote_endpoint_ =
			    native_socket.have_remote_endpoint();
	...
	boost::system::error_code bind(implementation_type &impl,
				       ...
				       boost::system::error_code &ec)
	{
		socket_ops::bind(impl.socket_, endpoint.data(), endpoint.size(),
				 ec);
		return ec;
	}
	...
	template <typename Option>
	boost::system::error_code
	get_option(const implementation_type &impl, Option &option,
				     ...
				     boost::system::error_code &ec) const
	{
		...
		if (socket_ops::getsockname(impl.socket_, endpoint.data(),
					    &addr_len, ec))
			   ...
			   socket_base::message_flags flags, Handler &handler)
	{
		...
		start_send_to_op(
		    impl, bufs.buffers(), bufs.count(), destination.data(),
				...
				Handler &handler)
	{
		...
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v)
		    op(sender_endp, impl.cancel_token_, buffers, handler);
	...
	boost::system::error_code
	accept(implementation_type &impl, Socket &peer,
	       endpoint_type *peer_endpoint, boost::system::error_code &ec)
	{
		...
		std::size_t addr_len =
		    peer_endpoint ? peer_endpoint->capacity() : 0;
		socket_holder new_socket(socket_ops::sync_accept(
		    impl.socket_, impl.state_,
	...
	{
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_accept_op<Socket, protocol_type,
						  Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		...
		p.p = new (p.v)
		    op(*this, impl.socket_, peer, impl.protocol_, peer_endpoint,
	...
	{
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_move_accept_op<protocol_type, Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		bool enable_connection_aborted =
			   ...
			   const endpoint_type &peer_endpoint, Handler &handler)
	{
		...
		start_connect_op(impl, impl.protocol_.family(),
