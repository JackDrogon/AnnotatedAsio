...
public:
	...
	BOOST_ASIO_DECL
	reactive_serial_port_service(boost::asio::io_context &io_context);
	...
	open(implementation_type &impl, const std::string &device,
	...
	boost::system::error_code
	get_option(const implementation_type &impl,
					     ...
					     boost::system::error_code &ec)
	{
		...
		descriptor_ops::error_wrapper(
		    ::tcsendbreak(descriptor_service_.native_handle(impl), 0),
		    ec);
		return ec;
	}
...
private:
	...
	store_option(const void *option, termios &storage,
		     boost::system::error_code &ec)
	{
		static_cast<const SettableSerialPortOption *>(option)->store(
		    storage, ec);
		return ec;
	}
	...
	BOOST_ASIO_DECL boost::system::error_code
	do_set_option(implementation_type &impl, store_function_type store,
	...
	template <typename GettableSerialPortOption>
	static boost::system::error_code
	load_option(void *option, const termios &storage,
		    boost::system::error_code &ec)
	{
		static_cast<GettableSerialPortOption *>(option)->load(storage,
								      ec);
		return ec;
	}
	...
	BOOST_ASIO_DECL boost::system::error_code
	do_get_option(const implementation_type &impl, load_function_type load,
