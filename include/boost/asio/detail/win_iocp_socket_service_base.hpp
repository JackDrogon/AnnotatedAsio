...
public:
	...
	// Constructor.
	BOOST_ASIO_DECL
	win_iocp_socket_service_base(boost::asio::io_context &io_context);
	...
	boost::system::error_code non_blocking(base_implementation_type &impl,
					       ...
					       boost::system::error_code &ec)
	{
		socket_ops::set_user_non_blocking(impl.socket_, impl.state_,
						  mode, ec);
		return ec;
	}
	...
	native_non_blocking(base_implementation_type &impl, bool mode,
			    boost::system::error_code &ec)
	{
		socket_ops::set_internal_non_blocking(impl.socket_, impl.state_,
						      mode, ec);
		return ec;
	}
				       ...
				       boost::system::error_code &ec)
	{
		...
		case socket_base::wait_read:
			socket_ops::poll_read(impl.socket_, impl.state_, -1,
					      ec);
			break;
		case socket_base::wait_write:
			socket_ops::poll_write(impl.socket_, impl.state_, -1,
					       ec);
			break;
		case socket_base::wait_error:
			socket_ops::poll_error(impl.socket_, impl.state_, -1,
					       ec);
			break;
		default:
			...
			socket_base::wait_type w, Handler &handler)
	{
		...
		BOOST_ASIO_HANDLER_CREATION((io_context_, *p.p, "socket", &impl,
		...
		default:
			...
			iocp_service_.post_immediate_completion(
			    p.p, is_continuation);
			break;
		}
	...
	size_t
	send(base_implementation_type &impl, const ConstBufferSequence &buffers,
	...
	{
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_send_op<ConstBufferSequence, Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
	...
	{
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_recv_op<MutableBufferSequence, Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v)
		    op(impl.state_, impl.cancel_token_, buffers, handler);
	...
	{
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_socket_recvmsg_op<MutableBufferSequence,
						   Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v)
		    op(impl.cancel_token_, buffers, out_flags, handler);
		...
		start_receive_op(impl, bufs.buffers(), bufs.count(), in_flags,
				 false, p.p);
		p.v = p.p = 0;
	}
				      ...
				      Handler &handler)
	{
		...
		BOOST_ASIO_HANDLER_CREATION(
		    (io_context_, *p.p, "socket", &impl, impl.socket_,
	...
	BOOST_ASIO_DECL void
	restart_accept_op(socket_type s, socket_holder &new_socket, int family,
...
protected:
	...
	BOOST_ASIO_DECL boost::system::error_code
	do_open(base_implementation_type &impl, int family, int type,
	...
	BOOST_ASIO_DECL void
	start_send_to_op(base_implementation_type &impl, WSABUF *buffers,
			 std::size_t buffer_count, const socket_addr_type *addr,
			 int addrlen, socket_base::message_flags flags,
			 operation *op);
	...
	BOOST_ASIO_DECL void
	start_receive_from_op(base_implementation_type &impl, WSABUF *buffers,
	...
	BOOST_ASIO_DECL void
	start_accept_op(base_implementation_type &impl, bool peer_is_open,
	...
	BOOST_ASIO_DECL void
	start_connect_op(base_implementation_type &impl, int family, int type,
	...
	// destroyed.
	BOOST_ASIO_DECL void
	close_for_destruction(base_implementation_type &impl);
	...
	typedef BOOL(PASCAL *connect_ex_fn)(SOCKET, const socket_addr_type *,
					    int, void *, DWORD, DWORD *,
					    OVERLAPPED *);
	...
	BOOST_ASIO_DECL connect_ex_fn
	get_connect_ex(base_implementation_type &impl, int type);
	...
	BOOST_ASIO_DECL void *interlocked_compare_exchange_pointer(void **dest,
								   void *exch,
								   void *cmp);
	...
	// - very old Platform SDKs; and
	// - platform SDKs where MSVC's /Wp64 option causes spurious warnings.
	BOOST_ASIO_DECL void *interlocked_exchange_pointer(void **dest,
							   void *val);
