...
public:
	...
	struct implementation_type : private boost::asio::detail::noncopyable {
		time_type expiry;
	...
	{
		scheduler_.move_timer(timer_queue_, impl.timer_data,
				      other_impl.timer_data);

		impl.expiry = other_impl.expiry;
		other_impl.expiry = time_type();

		impl.might_have_pending_waits =
		    other_impl.might_have_pending_waits;
			 ...
			 implementation_type &other_impl)
	{
		if (this != &other_service)
			if (impl.might_have_pending_waits)
				scheduler_.cancel_timer(timer_queue_,
							impl.timer_data);
	...
	{
		return Time_Traits::subtract(this->expiry(impl),
					     Time_Traits::now());
	}
	...
	{
		return expires_at(
		    impl, Time_Traits::add(Time_Traits::now(), expiry_time),
		    ec);
	}
	...
	void async_wait(implementation_type &impl, Handler &handler)
	{
		...
		BOOST_ASIO_HANDLER_CREATION((scheduler_.context(), *p.p,
					     "deadline_timer", &impl, 0,
					     "async_wait"));
