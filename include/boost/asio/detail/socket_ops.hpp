...
BOOST_ASIO_DECL void
complete_iocp_recv(state_type state, const weak_cancel_token_type &cancel_token,
...
non_blocking_recvfrom(socket_type s, buf *bufs, size_t count, int flags,
		      socket_addr_type *addr, std::size_t *addrlen,
...
BOOST_ASIO_DECL bool
non_blocking_sendto(socket_type s, const buf *bufs, size_t count, int flags,
...
getaddrinfo(const char *host, const char *service, const addrinfo_type &hints,
...
BOOST_ASIO_DECL boost::system::error_code
sync_getnameinfo(const socket_addr_type *addr, std::size_t addrlen, char *host,
		 std::size_t hostlen, char *serv, std::size_t servlen,
		 int sock_type, boost::system::error_code &ec);
