...
public:
	...
	winrt_async_manager(boost::asio::io_context &io_context)
	    : boost::asio::detail::service_base<winrt_async_manager>(
		  io_context),
		  ...
		  boost::system::error_code &ec)
	{
		...
		auto promise =
		    std::make_shared<std::promise<boost::system::error_code>>();
		     ...
		     boost::system::error_code &ec)
	{
		...
		operation->Completed = ref new AsyncOperationCompletedHandler<
		    TResult>([promise](IAsyncOperation<TResult> ^ operation,
				       AsyncStatus status) {
			switch (status) {
			case AsyncStatus::Canceled:
				promise->set_value(
	...
	TResult
	sync(Windows::Foundation::IAsyncOperationWithProgress<TResult,
							      TProgress> ^
		 operation,
	     boost::system::error_code &ec)
	{
		...
		operation->Completed =
		    ref new AsyncOperationWithProgressCompletedHandler<
			TResult, TProgress>([promise](
		   ...
		   winrt_async_op<TResult> *handler)
	{
		...
		auto on_completed = ref new AsyncOperationCompletedHandler<
		    TResult>(
	...
	template <typename TResult, typename TProgress>
	void async(Windows::Foundation::IAsyncOperationWithProgress<TResult,
								    TProgress> ^
		       operation,
		   winrt_async_op<TResult> *handler)
	{
		...
		auto on_completed =
		    ref new AsyncOperationWithProgressCompletedHandler<
			TResult, TProgress>([this, handler](
						IAsyncOperationWithProgress<
						    TResult, TProgress> ^
						    operation,
						AsyncStatus status) {
