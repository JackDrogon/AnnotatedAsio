...
class win_iocp_handle_service : public service_base<win_iocp_handle_service>
{
public:
	...
	public:
		...
		implementation_type()
		    : handle_(INVALID_HANDLE_VALUE),
		      safe_cancellation_thread_id_(0), next_(0), prev_(0)
		{
		}
	...
	BOOST_ASIO_DECL
	win_iocp_handle_service(boost::asio::io_context &io_context);
	...
	assign(implementation_type &impl, const native_handle_type &handle,
	...
	BOOST_ASIO_DECL boost::system::error_code
	cancel(implementation_type &impl, boost::system::error_code &ec);
	...
	size_t write_some_at(implementation_type &impl, uint64_t offset,
			     ...
			     boost::system::error_code &ec)
	{
		boost::asio::const_buffer buffer = buffer_sequence_adapter<
			      ...
			      Handler &handler)
	{
		...
		typedef win_iocp_handle_write_op<ConstBufferSequence, Handler>
		    op;
		...
		BOOST_ASIO_HANDLER_CREATION(
		    (iocp_service_.context(), *p.p, "handle", &impl,
	...
	size_t read_some_at(implementation_type &impl, uint64_t offset,
			    ...
			    boost::system::error_code &ec)
	{
		boost::asio::mutable_buffer buffer = buffer_sequence_adapter<
	...
	{
		// Allocate and construct an operation to wrap the handler.
		typedef win_iocp_handle_read_op<MutableBufferSequence, Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v) op(buffers, handler);
...
private:
	...
	start_write_op(implementation_type &impl, uint64_t offset,
		       const boost::asio::const_buffer &buffer, operation *op);

	// Helper function to perform a synchronous write operation.
	BOOST_ASIO_DECL size_t
	do_read(implementation_type &impl, uint64_t offset,
		const boost::asio::mutable_buffer &buffer,
		boost::system::error_code &ec);

	// Helper function to start a read operation.
	BOOST_ASIO_DECL void
	start_read_op(implementation_type &impl, uint64_t offset,
		      const boost::asio::mutable_buffer &buffer, operation *op);

	// Update the ID of the thread from which cancellation is safe.
	BOOST_ASIO_DECL void
	update_cancellation_thread_id(implementation_type &impl);
