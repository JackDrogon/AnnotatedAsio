...
public:
				...
				std::size_t /*bytes_transferred*/)
	{
		...
		if (owner) {
			...
			if (ec == boost::asio::error::connection_aborted &&
				...
				o->socket_service_.restart_accept_op(
				    o->socket_, o->new_socket_,
				    o->protocol_.family(), o->protocol_.type(),
				    o->protocol_.protocol(), o->output_buffer(),
				    o->address_length(), o);
				p.v = p.p = 0;
				return;
			}
		...
		detail::binder1<Handler, boost::system::error_code> handler(
		    o->handler_, ec);
...
public:
	...
	win_iocp_socket_move_accept_op(
	    ...
	    : operation(&win_iocp_socket_move_accept_op::do_complete),
	      socket_service_(socket_service), socket_(socket),
	      peer_(peer_io_context), protocol_(protocol),
				...
				std::size_t /*bytes_transferred*/)
	{
		...
		if (owner) {
			...
			if (!ec) {
				o->peer_.assign(
					...
					native_handle_type(o->new_socket_.get(),
							   peer_endpoint),
				    ec);
				if (!ec)
					o->new_socket_.release();
		...
		// to ensure that any owning sub-object remains valid until after we have
		// deallocated the memory here.
		detail::move_binder2<Handler, boost::system::error_code,
				     typename Protocol::socket>
		    handler(0, BOOST_ASIO_MOVE_CAST(Handler)(o->handler_), ec,
			    BOOST_ASIO_MOVE_CAST(typename Protocol::socket)(
				o->peer_));
		p.h = boost::asio::detail::addressof(handler.handler_);
		p.reset();
