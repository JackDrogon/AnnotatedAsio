...
public:
	...
	T *allocate(std::size_t n)
	{
		return static_cast<T *>(
		    boost_asio_handler_alloc_helpers::allocate(sizeof(T) * n,
							       handler_));
	}

	void deallocate(T *p, std::size_t n)
	{
		boost_asio_handler_alloc_helpers::deallocate(p, sizeof(T) * n,
							     handler_);
	}
	...
	struct ptr {                                                           \
		...
		{                                                              \
			...
			typedef typename ::boost::asio::detail::               \
			    get_hook_allocator<                                \
				Handler, associated_allocator_type>::type      \
				hook_allocator_type;                           \
			BOOST_ASIO_REBIND_ALLOC(hook_allocator_type, op)       \
			a(::boost::asio::detail::get_hook_allocator<           \
			    Handler, associated_allocator_type>::              \
			      get(handler,                                     \
		...
		{                                                              \
			...
			if (v) {                                               \
				typedef typename ::boost::asio::               \
				    associated_allocator<Handler>::type        \
					associated_allocator_type;             \
				typedef typename ::boost::asio::detail::       \
				    get_hook_allocator<                        \
					Handler,                               \
					associated_allocator_type>::type       \
					hook_allocator_type;                   \
				BOOST_ASIO_REBIND_ALLOC(hook_allocator_type,   \
							op)                    \
				a(::boost::asio::detail::get_hook_allocator<   \
				    Handler, associated_allocator_type>::      \
				      get(*h,                                  \
...
#define BOOST_ASIO_DEFINE_HANDLER_ALLOCATOR_PTR(op)                            \
	struct ptr {                                                           \
		...
		{                                                              \
			typedef typename ::boost::asio::detail::               \
			    get_recycling_allocator<Alloc>::type               \
				recycling_allocator_type;                      \
			BOOST_ASIO_REBIND_ALLOC(recycling_allocator_type, op)  \
			a1(::boost::asio::detail::get_recycling_allocator<     \
			    Alloc>::get(a));                                   \
		...
		{                                                              \
			...
			if (v) {                                               \
				...
				BOOST_ASIO_REBIND_ALLOC(                       \
				    recycling_allocator_type, op)              \
				a1(::boost::asio::detail::                     \
				       get_recycling_allocator<Alloc>::get(    \
					   *a));                               \
