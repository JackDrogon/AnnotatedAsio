...
public:
	...
	std::pair<iterator, bool> insert(const value_type &v)
	{
		...
		if (it == values_.end()) {
			...
			++size_;
			return std::pair<iterator, bool>(buckets_[bucket].last,
							 true);
		}
		iterator end_it = buckets_[bucket].last;
		++end_it;
...
private:
	...
	static std::size_t hash_size(std::size_t num_elems)
	{
		...
		const std::size_t nth_size =
		    sizeof(sizes) / sizeof(std::size_t) - 1;
	...
	void rehash(std::size_t num_buckets)
	{
		...
		while (iter != end_iter) {
			std::size_t bucket =
			    calculate_hash_value(iter->first) % num_buckets_;
			if (buckets_[bucket].last == end_iter) {
				...
				values_.splice(buckets_[bucket].last, values_,
					       iter++);
				--buckets_[bucket].last;
			}
		}
