...
public:
				     ...
				     boost::system::error_code &ec) const
	{
		...
		endpoint.resize(do_get_endpoint(impl, true, endpoint.data(),
						endpoint.size(), ec));
		return endpoint;
	}
	...
	{
		endpoint_type endpoint;
		endpoint.resize(do_get_endpoint(impl, false, endpoint.data(),
						endpoint.size(), ec));
		return endpoint;
	}
			   ...
			   const endpoint_type &peer_endpoint, Handler &handler)
	{
		...
		start_connect_op(impl, peer_endpoint.data(), p.p,
				 is_continuation);
		p.v = p.p = 0;
	}
};
