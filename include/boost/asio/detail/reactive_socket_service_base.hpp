...
public:
			...
			socket_base::wait_type w, Handler &handler)
	{
		...
		default:
			...
			reactor_.post_immediate_completion(p.p,
							   is_continuation);
			p.v = p.p = 0;
			return;
		}
			...
			socket_base::message_flags flags, Handler &handler)
	{
		...
		// Allocate and construct an operation to wrap the handler.
		typedef reactive_socket_send_op<ConstBufferSequence, Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v)
		    op(impl.socket_, impl.state_, buffers, flags, handler);
			...
			socket_base::message_flags, Handler &handler)
	{
		...
		start_op(impl, reactor::write_op, p.p, is_continuation, false,
			 false);
		p.v = p.p = 0;
	}
			   ...
			   socket_base::message_flags flags, Handler &handler)
	{
		...
		// Allocate and construct an operation to wrap the handler.
		typedef reactive_socket_recv_op<MutableBufferSequence, Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v)
				      ...
				      Handler &handler)
	{
		...
		// Allocate and construct an operation to wrap the handler.
		typedef reactive_socket_recvmsg_op<MutableBufferSequence,
						   Handler>
		    op;
		typename op::ptr p = {boost::asio::detail::addressof(handler),
				      op::ptr::allocate(handler), 0};
		p.p = new (p.v)
		    op(impl.socket_, buffers, in_flags, out_flags, handler);
				      ...
				      Handler &handler)
	{
		...
		BOOST_ASIO_HANDLER_CREATION(
		    (reactor_.context(), *p.p, "socket", &impl, impl.socket_,
...
protected:
	...
	BOOST_ASIO_DECL void start_op(base_implementation_type &impl,
