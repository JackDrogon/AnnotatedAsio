...
template <typename Executor> class awaiter
{
public:
	struct deleter {
	...
	awaiter *get_return_object() { return this; }
	...
	{
		if (--ref_count_ == 0)
			coroutine_handle<awaiter>::from_promise(*this)
			    .destroy();
	}
	...
	void rethrow_unhandled_exception()
	{
		if (pending_exception_) {
			std::exception_ptr ex =
			    std::exchange(pending_exception_, nullptr);
...
template <typename Executor> class awaitee_base
{
public:
	...
	void *operator new(std::size_t size)
	{
		return boost::asio::detail::thread_info_base::allocate(
		    ...
		    boost::asio::detail::thread_context::thread_call_stack::
			top(),
		    size);
	}
	...
	auto final_suspend() { return final_suspender{this}; }

	void set_except(std::exception_ptr e) { pending_exception_ = e; }
	...
	awaiter<Executor> *top() { return awaiter_; }

	coroutine_handle<void> caller() { return caller_; }
	...
	public:
		...
		void await_suspend(
		    coroutine_handle<detail::awaitee<U, Ex>> h) noexcept
		{
...
template <typename Executor> class awaiter_task
{
public:
	typedef Executor executor_type;
...
template <typename Executor>
class await_handler<Executor, void> : public await_handler_base<Executor, void>
{
public:
...
public:
	...
	void operator()(const boost::system::error_code &ec)
	{
		typename awaiter<Executor>::ptr ptr(std::move(this->awaiter_));
		if (ec) {
			this->awaitee_->set_except(std::make_exception_ptr(
...
template <typename Executor, typename T>
class await_handler<Executor, T> : public await_handler_base<Executor, T>
{
public:
...
class await_handler : public await_handler_base<Executor, std::tuple<Ts...>>
{
public:
...
co_spawn_entry_point(awaitable<T, Executor> *,
		     executor_work_guard<Executor> work_guard, F f,
		     Handler handler)
{
	bool done = false;
...
template <typename Awaitable> inline Awaitable make_dummy_awaitable()
{
	for (;;)
...
public:
	...
	return_type get() { return std::move(awaitable_); }
...
public:
	...
	type get() { return std::move(awaitable_); }
...
template <typename Executor, typename... Args>
struct coroutine_traits<boost::asio::experimental::detail::awaiter<Executor> *,
			Args...> {
	typedef boost::asio::experimental::detail::awaiter<Executor>
	    promise_type;
};

template <typename T, typename Executor, typename... Args>
struct coroutine_traits<boost::asio::experimental::awaitable<T, Executor>,
			Args...> {
	typedef boost::asio::experimental::detail::awaitee<T, Executor>
	    promise_type;
};
