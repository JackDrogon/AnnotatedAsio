 ...
 */
template <typename Executor> class await_token
{
public:
...
template <typename T, typename Executor = strand<executor>> class awaitable
{
public:
	...
	{
		if (awaitee_) {
			detail::coroutine_handle<detail::awaitee<T, Executor>>::
			    from_promise(*awaitee_)
				.destroy();
		}
	}
	...
	bool await_ready() const noexcept { return awaitee_->ready(); }
	...
	// Support for co_await keyword.
	T await_resume() { return awaitee_->get(); }
...
/// Spawn a new thread of execution.
template <typename Executor, typename F, typename CompletionToken>
inline auto co_spawn(const await_token<Executor> &parent, F &&f,
