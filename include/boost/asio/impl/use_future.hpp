...
template <typename T> class promise_executor
{
public:
...
{
public:
	template <typename Arg> void operator()(BOOST_ASIO_MOVE_ARG(Arg) arg)
	{
...
public:
			...
			BOOST_ASIO_MOVE_ARG(Args)... args)
	{
		if (ec) {
			...
			this->p_->set_value(std::forward_as_tuple(
...
{
	return detail::packaged_token<typename decay<Function>::type,
				      Allocator>(
...
public:
	explicit async_result(
		...
		Function, Allocator,
		typename result_of<Function()>::type>::completion_handler_type
		&h)
	    : detail::packaged_async_result<
...
public:
	typedef
	    typename detail::promise_async_result<Signature,
						  Allocator>::return_type type;
...
struct handler_type<detail::packaged_token<Function, Allocator>, Signature> {
	typedef
	    typename async_result<detail::packaged_token<Function, Allocator>,
				  Signature>::completion_handler_type type;
};
