...
public:
	explicit async_result(typename detail::coro_async_result<
			      Handler, void>::completion_handler_type &h)
...
public:
	explicit async_result(
		...
		Handler, typename decay<Arg1>::type>::completion_handler_type
		&h)
...
public:
	explicit async_result(
	    typename detail::coro_async_result<
		Handler, typename decay<Arg2>::type>::completion_handler_type
		&h)
...
public:
	typedef
	    typename detail::coro_async_result<Handler, T>::return_type type;

	explicit async_result(typename detail::coro_async_result<
			      Handler, T>::completion_handler_type &h)
...
struct associated_allocator<detail::coro_handler<Handler, T>, Allocator> {
	...
	{
		return associated_allocator<Handler, Allocator>::get(h.handler_,
								     a);
	}
};
...
struct associated_executor<detail::coro_handler<Handler, T>, Executor> {
	...
	{
		return associated_executor<Handler, Executor>::get(h.handler_,
								   ex);
	}
};
...
struct spawn_data : private noncopyable {
	...
	spawn_data(BOOST_ASIO_MOVE_ARG(Hand) handler, bool call_handler,
...
template <typename Handler, typename Function> struct spawn_helper {
	void operator()()
	{
		typedef typename basic_yield_context<Handler>::callee_type
		    callee_type;
		coro_entry_point<Handler, Function> entry_point = {data_};
		shared_ptr<callee_type> coro(
	   ...
	   const boost::coroutines::attributes &attributes)
{
	...
	Handler handler(
	    ctx.handler_); // Explicit copy that might be moved from.
...
inline void spawn(const Executor &ex, BOOST_ASIO_MOVE_ARG(Function) function,
		  const boost::coroutines::attributes &attributes,
...
{
	boost::asio::spawn(
	    boost::asio::bind_executor(ex, &detail::default_spawn_handler),
...
{
	boost::asio::spawn(
	    boost::asio::bind_executor(s, &detail::default_spawn_handler),
	    BOOST_ASIO_MOVE_CAST(Function)(function), attributes);
}

template <typename Function, typename ExecutionContext>
inline void spawn(
    ExecutionContext &ctx, BOOST_ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes &attributes,
    typename enable_if<
