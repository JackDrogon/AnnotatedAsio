				  ...
				  boost::system::error_code &ec)
{
	...
	boost::asio::detail::consuming_buffers<
	    const_buffer, ConstBufferSequence, ConstBufferIterator>
	    tmp(buffers);
	while (!tmp.empty()) {
...
template <typename SyncWriteStream, typename DynamicBuffer,
	  typename CompletionCondition>
std::size_t write(
    SyncWriteStream &s, BOOST_ASIO_MOVE_ARG(DynamicBuffer) buffers,
    CompletionCondition completion_condition, boost::system::error_code &ec,
    typename enable_if<
	...
	is_dynamic_buffer<typename decay<DynamicBuffer>::type>::value>::type *)
{
	boost::system::error_code ec;
	std::size_t bytes_transferred =
	    write(s, BOOST_ASIO_MOVE_CAST(DynamicBuffer)(buffers),
		  transfer_all(), ec);
	boost::asio::detail::throw_error(ec, "write");
	return bytes_transferred;
}
...
template <typename SyncWriteStream, typename Allocator,
	  typename CompletionCondition>
inline std::size_t
write(SyncWriteStream &s, boost::asio::basic_streambuf<Allocator> &b,
      CompletionCondition completion_condition, boost::system::error_code &ec)
{
    ...
    WriteHandler &handler)
{
	detail::write_op<AsyncWriteStream, ConstBufferSequence,
			 ...
			 WriteHandler>(stream, buffers, completion_condition,
...
struct associated_allocator<
    ...
    Allocator> {
	typedef
	    typename associated_allocator<WriteHandler, Allocator>::type type;
	...
	{
		return associated_allocator<WriteHandler, Allocator>::get(
		    h.handler_, a);
	}
};
...
struct associated_executor<
	...
	{
		return associated_executor<WriteHandler, Executor>::get(
		    h.handler_, ex);
	}
};
	    ...
	    is_const_buffer_sequence<ConstBufferSequence>::value>::type *)
{
	...
	detail::start_write_buffer_sequence_op(
	    s, buffers, boost::asio::buffer_sequence_begin(buffers),
...
public:
	...
	write_dynbuf_op(AsyncWriteStream &stream,
	    ...
	    : stream_(stream),
	      ...
	      completion_condition_(BOOST_ASIO_MOVE_CAST(CompletionCondition)(
		  completion_condition)),
	      handler_(BOOST_ASIO_MOVE_CAST(WriteHandler)(handler))
	{
	}
			...
			std::size_t bytes_transferred, int start = 0)
	{
		...
		case 1:
			async_write(
			    stream_, buffers_.data(), completion_condition_,
...
inline void *asio_handler_allocate(
    std::size_t size,
    write_dynbuf_op<AsyncWriteStream, DynamicBuffer, CompletionCondition,
...
template <typename Function, typename AsyncWriteStream, typename DynamicBuffer,
	  typename CompletionCondition, typename WriteHandler>
inline void asio_handler_invoke(
    Function &function,
    write_dynbuf_op<AsyncWriteStream, DynamicBuffer, CompletionCondition,
...
template <typename AsyncWriteStream, typename DynamicBuffer,
	  ...
	  typename Allocator>
struct associated_allocator<
    detail::write_dynbuf_op<AsyncWriteStream, DynamicBuffer,
			    CompletionCondition, WriteHandler>,
    Allocator> {
	typedef
	    typename associated_allocator<WriteHandler, Allocator>::type type;
	...
	static type
	get(const detail::write_dynbuf_op<AsyncWriteStream, DynamicBuffer,
...
inline BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler,
				     void(boost::system::error_code,
					  std::size_t))
    async_write(AsyncWriteStream &s, BOOST_ASIO_MOVE_ARG(DynamicBuffer) buffers,
		BOOST_ASIO_MOVE_ARG(WriteHandler) handler,
		typename enable_if<is_dynamic_buffer<
		    typename decay<DynamicBuffer>::type>::value>::type *)
{
	return async_write(s, BOOST_ASIO_MOVE_CAST(DynamicBuffer)(buffers),
			   transfer_all(),
		    ...
		    typename decay<DynamicBuffer>::type>::value>::type *)
{
	...
	detail::write_dynbuf_op<
	    AsyncWriteStream, typename decay<DynamicBuffer>::type,
	    CompletionCondition,
...
inline BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler,
				     void(boost::system::error_code,
					  std::size_t))
    async_write(AsyncWriteStream &s, boost::asio::basic_streambuf<Allocator> &b,
		BOOST_ASIO_MOVE_ARG(WriteHandler) handler)
{
