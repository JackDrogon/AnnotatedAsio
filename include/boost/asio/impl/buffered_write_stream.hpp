...
template <typename WriteHandler>
inline bool
asio_handler_is_continuation(buffered_flush_handler<WriteHandler> *this_handler)
{
...
buffered_write_stream<Stream>::async_flush(BOOST_ASIO_MOVE_ARG(WriteHandler)
					       handler)
{
...
public:
	...
	void operator()(const boost::system::error_code &ec, std::size_t)
	{
		if (ec) {
			...
			std::size_t space_avail =
			    storage_.capacity() - orig_size;
