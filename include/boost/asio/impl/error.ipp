...
public:
	...
	std::string message(int value) const
	{
		...
		if (value == error::host_not_found_try_again)
			return "Host not found (non-authoritative), try again "
			       "later";
		if (value == error::no_data)
			return "The query is valid, but it does not have "
			       "associated data";
		if (value == error::no_recovery)
			return "A non-recoverable error occurred during "
			       "database lookup";
...
public:
	...
	std::string message(int value) const
	{
		...
		if (value == error::fd_set_failure)
			return "The descriptor does not fit into the select "
			       "call's fd_set";
