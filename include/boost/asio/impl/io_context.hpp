...
template <typename Service> inline Service &use_service(io_context &ioc)
{
	...
	(void)static_cast<execution_context::service *>(
	    static_cast<Service *>(0));
...
io_context::run_one_until(const chrono::time_point<Clock, Duration> &abs_time)
{
	...
	while (now < abs_time) {
		...
		std::size_t s = impl_.wait_one(
		    static_cast<long>(
			chrono::duration_cast<chrono::microseconds>(rel_time)
			    .count()),
		    ec);
...
io_context::dispatch(BOOST_ASIO_MOVE_ARG(LegacyCompletionHandler) handler)
{
	...
	BOOST_ASIO_LEGACY_COMPLETION_HANDLER_CHECK(LegacyCompletionHandler,
						   handler)
	type_check;
	...
	if (impl_.can_dispatch()) {
		...
		typedef detail::completion_handler<typename handler_type<
		    LegacyCompletionHandler, void()>::type>
		    op;
		typename op::ptr p = {
		    detail::addressof(init.completion_handler),
...
io_context::post(BOOST_ASIO_MOVE_ARG(LegacyCompletionHandler) handler)
{
	...
	BOOST_ASIO_HANDLER_CREATION(
	    (*this, *p.p, "io_context", this, 0, "post"));
					 ...
					 const Allocator &a) const
{
	...
	typedef detail::executor_op<function_type, Allocator, detail::operation>
	    op;
				      ...
				      const Allocator &a) const
{
	...
	BOOST_ASIO_HANDLER_CREATION((this->context(), *p.p, "io_context",
				     &this->context(), 0, "defer"));
