...
deref_connect_result(Iterator iter, boost::system::error_code &ec)
{
...
template <typename T, typename Iterator> struct is_legacy_connect_condition {
	...
	static const bool value =
		...
		(*static_cast<legacy_connect_condition_helper<T, Iterator> *>(
		    0))(*static_cast<const boost::system::error_code *>(0),
...
call_connect_condition(ConnectCondition &connect_condition,
		       const boost::system::error_code &ec, Iterator next,
		       Iterator end,
...
Iterator
connect(basic_socket<Protocol BOOST_ASIO_SVC_TARG> &s, Iterator begin,
...
connect(basic_socket<Protocol BOOST_ASIO_SVC_TARG> &s, Iterator begin,
	...
	typename enable_if<!is_endpoint_sequence<Iterator>::value>::type *)
{
	return connect(s, begin, Iterator(),
		       detail::default_connect_condition(), ec);
}
...
{
	return detail::deref_connect_result<Protocol>(
	    connect(s, endpoints.begin(), endpoints.end(), connect_condition,
		    ec),
	    ec);
}
		 ...
		 boost::system::error_code &ec)
{
	...
	for (Iterator iter = begin; iter != end; ++iter) {
		iter = (detail::call_connect_condition(connect_condition, ec,
						       iter, end));
		if (iter != end) {
			s.close(ec);
			s.connect(*iter, ec);
...
protected:
	...
	{
		iter = detail::call_connect_condition(connect_condition_, ec,
						      iter, end);
	}
...
protected:
	...
	template <typename Iterator>
	void check_condition(const boost::system::error_code &, Iterator &,
			     Iterator &)
	{
	}
};
...
public:
	...
	range_connect_op(range_connect_op &&other)
	    : base_from_connect_condition<ConnectCondition>(other),
	      ...
	      index_(other.index_), start_(other.start_),
	      handler_(
		  BOOST_ASIO_MOVE_CAST(RangeConnectHandler)(other.handler_))
	{
	}
	...
	{
		typename EndpointSequence::const_iterator begin =
		    endpoints_.begin();
		typename EndpointSequence::const_iterator iter = begin;
		std::advance(iter, index_);
		typename EndpointSequence::const_iterator end =
		    endpoints_.end();
...
template <typename Function, typename Protocol BOOST_ASIO_SVC_TPARAM,
...
public:
	...
	iterator_connect_op(iterator_connect_op &&other)
	    : base_from_connect_condition<ConnectCondition>(other),
	      socket_(other.socket_), iter_(other.iter_), end_(other.end_),
	      start_(other.start_), handler_(BOOST_ASIO_MOVE_CAST(
					IteratorConnectHandler)(other.handler_))
	{
	}
#endif // defined(BOOST_ASIO_HAS_MOVE)
...
struct associated_allocator<
    ...
    Allocator> {
	typedef
	    typename associated_allocator<RangeConnectHandler, Allocator>::type
		type;

	static type
	get(const detail::range_connect_op<Protocol BOOST_ASIO_SVC_TARG,
					   EndpointSequence, ConnectCondition,
...
struct associated_executor<
    ...
    Executor> {
	typedef
	    typename associated_executor<RangeConnectHandler, Executor>::type
		type;
	...
	{
		return associated_executor<RangeConnectHandler, Executor>::get(
		    h.handler_, ex);
	}
};
...
struct associated_allocator<
    ...
    Allocator> {
	typedef typename associated_allocator<IteratorConnectHandler,
					      Allocator>::type type;

	static type
	get(const detail::iterator_connect_op<Protocol BOOST_ASIO_SVC_TARG,
...
struct associated_executor<
    ...
    Executor> {
	typedef
	    typename associated_executor<IteratorConnectHandler, Executor>::type
		type;
	    ...
	    *)
{
	...
	BOOST_ASIO_RANGE_CONNECT_HANDLER_CHECK(RangeConnectHandler, handler,
					       typename Protocol::endpoint)
	type_check;

	async_completion<RangeConnectHandler, void(boost::system::error_code,
						   typename Protocol::endpoint)>
	    init(handler);
...
inline BOOST_ASIO_INITFN_RESULT_TYPE(IteratorConnectHandler,
    ...
    async_connect(
	basic_socket<Protocol BOOST_ASIO_SVC_TARG> &s, Iterator begin,
	BOOST_ASIO_MOVE_ARG(IteratorConnectHandler) handler,
	typename enable_if<!is_endpoint_sequence<Iterator>::value>::type *)
{
	// If you get an error on the following line it means that your handler does
	// not meet the documented type requirements for a IteratorConnectHandler.
	BOOST_ASIO_ITERATOR_CONNECT_HANDLER_CHECK(IteratorConnectHandler,
						  handler, Iterator)
	type_check;
	...
	detail::iterator_connect_op<
	    Protocol BOOST_ASIO_SVC_TARG, Iterator,
	    detail::default_connect_condition,
	    ...
	    *)
{
	...
	detail::range_connect_op<
	    Protocol BOOST_ASIO_SVC_TARG, EndpointSequence, ConnectCondition,
	    BOOST_ASIO_HANDLER_TYPE(
		RangeConnectHandler,
	...
	typename enable_if<!is_endpoint_sequence<Iterator>::value>::type *)
{
	...
	detail::iterator_connect_op<
	    Protocol BOOST_ASIO_SVC_TARG, Iterator, ConnectCondition,
	    BOOST_ASIO_HANDLER_TYPE(IteratorConnectHandler,
				    void(boost::system::error_code, Iterator))>(
