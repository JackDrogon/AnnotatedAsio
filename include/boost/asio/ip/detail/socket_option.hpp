	...
	void resize(const Protocol &protocol, std::size_t s)
	{
		if (protocol.family() == PF_INET6) {
			if (s != sizeof(ipv6_value_)) {
				std::length_error ex(
				    "multicast_enable_loopback socket option "
				    "resize");
...
public:
	      ...
	      ipv6_value_()  // Zero-initialisation gives the "any" address.
	{
		if (multicast_address.is_v6()) {
			...
			memcpy(ipv6_value_.ipv6mr_multiaddr.s6_addr,
			       bytes.data(), 16);
			...
			ipv4_value_.imr_interface.s_addr = boost::asio::detail::
			    socket_ops::host_to_network_long(
	    ...
	    : ipv4_value_() // Zero-initialisation gives the "any" address.
	{
		...
		if (network_interface)
			...
			ipv6_value_.ipv6mr_interface =
			    multicast_address.scope_id();
	}
