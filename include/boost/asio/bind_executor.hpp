...
template <typename T> struct executor_binder_check {
	typedef void type;
};
...
struct executor_binder_result_type<
    T, typename executor_binder_check<typename T::result_type>::type> {
...
public:
   ...
   */
	...
	executor_binder(executor_binder &&other)
	    : base_type(
		  BOOST_ASIO_MOVE_CAST(executor_type)(other.get_executor()),
	...
	executor_binder(executor_binder<U, OtherExecutor> &&other)
	    : base_type(
		  BOOST_ASIO_MOVE_CAST(OtherExecutor)(other.get_executor()),
	...
	target_type &get() BOOST_ASIO_NOEXCEPT { return this->target_; }
	...
	operator()(BOOST_ASIO_MOVE_ARG(Args)... args)
	{
	...
	template <typename... Args>
	typename result_of<T(Args...)>::type
	operator()(BOOST_ASIO_MOVE_ARG(Args)... args) const
	{
		return this->target_(BOOST_ASIO_MOVE_CAST(Args)(args)...);
	}
...
#undef BOOST_ASIO_PRIVATE_BIND_EXECUTOR_CALL_DEF
	...
	typedef
	    typename detail::executor_binder_result_type<T>::result_type_or_void
		result_type_or_void;
...
#define BOOST_ASIO_PRIVATE_BIND_EXECUTOR_CALL_DEF(n)                           \
	template <BOOST_ASIO_VARIADIC_TPARAMS(n)>                              \
	result_type_or_void operator()(BOOST_ASIO_VARIADIC_MOVE_PARAMS(n))     \
	{                                                                      \
		return this->target_(BOOST_ASIO_VARIADIC_MOVE_ARGS(n));        \
	}                                                                      \
                                                                               \
...
struct handler_type<executor_binder<T, Executor>, Signature> {
	typedef executor_binder<typename handler_type<T, Signature>::type,
				Executor>
	    type;
};
