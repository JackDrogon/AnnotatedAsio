...
public:
	chat_session(tcp::socket socket, chat_room &room)
	    : socket_(std::move(socket)),
	      timer_(socket_.get_executor().context()), room_(room)
	{
		...
		try {
			for (std::string read_msg;;) {
				std::size_t n =
					...
					socket_,
					boost::asio::dynamic_buffer(read_msg,
								    1024),
					"\n", token);
...
awaitable<void> listener(tcp::acceptor acceptor)
{
	...
	for (;;) {
		std::make_shared<chat_session>(
		    co_await acceptor.async_accept(token), room)
		    ->start();
	}
}
...
int main(int argc, char *argv[])
{
	try {
		...
		for (int i = 1; i < argc; ++i) {
			...
			co_spawn(io_context,
				 [&] {
					 return listener(tcp::acceptor(
					     io_context, {tcp::v4(), port}));
				 },
				 detached);
		}
