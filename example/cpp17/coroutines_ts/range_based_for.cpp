...
public:
	tcp::socket operator*() { return std::move(socket_); }
	...
	bool operator!=(const connection_iter &) const noexcept { return true; }
};
...
public:
	...
	connection_iter end()
	{
		return connection_iter(
		    acceptor_, tcp::socket(acceptor_.get_executor().context()));
	}
};
...
awaitable<void> listener(tcp::acceptor acceptor)
{
  ...
  for
	  co_await(tcp::socket s : connections(acceptor))
	  {
		  co_await boost::asio::async_write(
		      s, boost::asio::buffer("hello\r\n", 7), token);
	  }
}
