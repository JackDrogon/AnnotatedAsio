...
class logger_service : public boost::asio::io_context::service
{
public:
	...
	logger_service(boost::asio::io_context &io_context)
	    : boost::asio::io_context::service(io_context), work_io_context_(),
	      work_(boost::asio::make_work_guard(work_io_context_)),
	      work_thread_(new boost::thread(boost::bind(
		  &boost::asio::io_context::run, &work_io_context_)))
	{
	}
	...
	void log(impl_type &impl, const std::string &message)
	{
		...
		boost::asio::post(
		    work_io_context_,
		    boost::bind(&logger_service::log_impl, this, os.str()));
	}
