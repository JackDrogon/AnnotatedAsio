...
private:
	void handle_connect(const boost::system::error_code &error)
	{
		if (!error) {
			...
			boost::asio::async_read(
			    ...
			    boost::bind(&posix_chat_client::handle_read_header,
					this,
					boost::asio::placeholders::error));
			...
			boost::asio::async_read_until(
			    ...
			    boost::bind(
				&posix_chat_client::handle_read_input, this,
				boost::asio::placeholders::error,
	...
	void handle_read_header(const boost::system::error_code &error)
	{
		if (!error && read_msg_.decode_header()) {
			...
			boost::asio::async_read(
			    ...
			    boost::bind(&posix_chat_client::handle_read_body,
					this,
					boost::asio::placeholders::error));
		} else {
			close();
	...
	void handle_read_body(const boost::system::error_code &error)
	{
		if (!error) {
			...
			boost::asio::async_write(
			    output_, buffers,
			    boost::bind(&posix_chat_client::handle_write_output,
					this,
					boost::asio::placeholders::error));
		} else {
			close();
			       ...
			       std::size_t length)
	{
		if (!error) {
			...
			input_buffer_.sgetn(write_msg_.body(),
					    input_buffer_.size());
			write_msg_.encode_header();
			boost::asio::async_write(
			    socket_,
