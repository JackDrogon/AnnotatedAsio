...
int main(int argc, char *argv[])
{
	try {
		...
		tcp::endpoint remote_endpoint =
		    *resolver.resolve(host_name, port).begin();
		...
		control_request start =
		    control_request::start(data_endpoint.port());
		...
		for (;;) {
			...
			boost::shared_ptr<udp::socket> new_data_socket(
			    new udp::socket(io_context,
					    udp::endpoint(udp::v4(), 0)));

			// Determine the new port we will use to receive data.
			udp::endpoint new_data_endpoint =
			    new_data_socket->local_endpoint();
			...
			boost::asio::async_write(
			    control_socket, change.to_buffers(),
			...
			new_data_socket->async_receive(
				...
				lambda::if_(!lambda::_1)[
				    ...
				    lambda::var(data_socket) =
					boost::shared_ptr<udp::socket>()]));
