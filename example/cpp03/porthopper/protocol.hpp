...
public:
	...
	static const control_request change(unsigned short old_port,
...
private:
			...
			unsigned short new_port_number)
	{
		...
		os << std::setw(encoded_port_size) << std::hex
		   << old_port_number;
		os << std::setw(encoded_port_size) << std::hex
		   << new_port_number;
	}
	...
	enum { encoded_port_size = 4, // 16-bit port in hex.
...
public:
	...
	frame(unsigned long frame_number, const std::string &payload_data)
	{
		...
		os << std::setw(encoded_number_size) << std::hex
		   << frame_number;
		os << std::setw(payload_size) << std::setfill(' ')
	...
	unsigned long number() const
	{
		...
		is >> std::setw(encoded_number_size) >> std::hex >>
		    frame_number;
		return frame_number;
	}
...
private:
	...
	enum { encoded_number_size = 8, // Frame number in hex.
