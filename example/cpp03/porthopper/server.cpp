...
public:
				    ...
				    control_request_ptr request)
	{
		if (!ec) {
			...
			timer_ptr delay_timer(new boost::asio::steady_timer(
			    acceptor_.get_executor().context()));
			delay_timer->expires_after(
			    boost::asio::chrono::seconds(2));
			delay_timer->async_wait(
			    boost::bind(&server::handle_control_request_timer,
					this, socket, request, delay_timer));
		}
	}
					  ...
					  timer_ptr /*delay_timer*/)
	{
		...
		if (!ec) {
			...
			if (unsigned short old_port = request->old_port()) {
				udp::endpoint old_endpoint(
				    remote_endpoint.address(), old_port);
				subscribers_.erase(old_endpoint);
				std::cout << "Removing subscription "
					  << old_endpoint << std::endl;
			}

			// Add new port subscription, if any.
			if (unsigned short new_port = request->new_port()) {
				udp::endpoint new_endpoint(
				    remote_endpoint.address(), new_port);
	...
	void handle_timer()
	{
		...
		double y = std::sin(x);
		int char_index =
		    static_cast<int>((y + 1.0) * (frame::payload_size / 2));
		std::string payload;
