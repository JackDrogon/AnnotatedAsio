...
public:
	explicit session(boost::asio::io_context &io_context)
	    : strand_(io_context), socket_(io_context), timer_(io_context)
	{
	}
	...
	void go()
	{
		boost::asio::spawn(
		    strand_,
		    boost::bind(&session::echo, shared_from_this(), _1));
		boost::asio::spawn(
		    strand_,
		    boost::bind(&session::timeout, shared_from_this(), _1));
	}
		...
		try {
			...
			for (;;) {
				...
				boost::asio::async_write(
				    socket_, boost::asio::buffer(data, n),
				    yield);
			}
...
void do_accept(boost::asio::io_context &io_context, unsigned short port,
...
int main(int argc, char *argv[])
{
	try {
		...
		boost::asio::spawn(
		    io_context, boost::bind(do_accept, boost::ref(io_context),
					    atoi(argv[1]), _1));
