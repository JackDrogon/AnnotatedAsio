...
{
public:
	session(tcp::socket &socket) : socket_(socket), state_(reading) {}
	...
	void do_write(boost::system::error_code &ec)
	{
		if (std::size_t len = socket_.write_some(
			...
			state_ = boost::asio::buffer_size(write_buffer_) > 0
				     ? writing
				     : reading;
		}
	}
...
private:
	connection(boost::asio::io_context &io_context)
	    : socket_(io_context), session_impl_(socket_),
	      read_in_progress_(false), write_in_progress_(false)
	{
	}
