...
struct close_after {
	close_after(boost::asio::chrono::steady_clock::duration t,
		    tcp::socket &s)
...
public:
	...
	explicit async_result(completion_handler_type &h)
	    : timeout_(h.token_.timeout_), socket_(h.token_.socket_)
	{
		h.ec_ = &ec_;
		h.t_ = &t_;
	}
	...
	try {
		...
		for (std::string input_buffer;;) {
			...
			std::size_t n = boost::asio::async_read_until(
			    socket, boost::asio::dynamic_buffer(input_buffer),
			    '\n',
			    close_after(boost::asio::chrono::seconds(10),
					socket));
