...
public:
	tcp_session(boost::asio::io_context &io_context, channel &ch)
	    : channel_(ch), socket_(io_context), input_deadline_(io_context),
	      non_empty_output_queue_(io_context), output_deadline_(io_context)
	{
...
private:
	...
	void await_output()
	{
		...
		if (output_queue_.empty()) {
			...
			non_empty_output_queue_.async_wait(boost::bind(
			    &tcp_session::await_output, shared_from_this()));
		} else {
			start_write();
		}
	}
	...
	void start_write()
	{
		...
		output_deadline_.expires_after(
		    boost::asio::chrono::seconds(30));
...
public:
	...
	{
		subscriber_ptr bc(
		    new udp_broadcaster(io_context_, broadcast_endpoint));
		channel_.join(bc);
	...
	void start_accept()
	{
		tcp_session_ptr new_session(
		    new tcp_session(io_context_, channel_));
