...
void transmit_file(tcp::socket &socket, random_access_handle &file,
		   Handler handler)
{
	...
	BOOL ok = ::TransmitFile(socket.native_handle(), file.native_handle(),
				 0, 0, overlapped.get(), 0, 0);
...
public:
	...
	void start()
	{
		...
		file_.assign(
		    ::CreateFile(
			filename_.c_str(), GENERIC_READ, 0, 0, OPEN_ALWAYS,
...
private:
	connection(boost::asio::io_context &io_context,
		   const std::string &filename)
	    : socket_(io_context), filename_(filename), file_(io_context)
	{
	}
...
private:
	void start_accept()
	{
		connection::pointer new_connection = connection::create(
		    acceptor_.get_executor().context(), filename_);
