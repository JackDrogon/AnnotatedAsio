...
public:
	    ...
	    : use_count_(1), threads_(thread_count)
	{
		try {
			...
			for (thread_count_ = 0; thread_count_ < thread_count;
			     ++thread_count_) {
				dispatch(threads_, [&] {
...
private:
	...
	{
		std::shared_ptr<std::size_t> work_count(
		    std::move(p->work_count_));
		try {
			p->execute_(p);
			lock.lock();
...
private:
	template <class Func> struct function : fork_join_pool::function_base {
		    ...
		    : function_(std::move(f))
		{
			work_count_ = w;
			execute_ =
			    [](std::shared_ptr<fork_join_pool::function_base>
				   &p) {
				    Func tmp(std::move(
					static_cast<function *>(p.get())
					    ->function_));
				    p.reset();
				    tmp();
			    };
	...
	if (n > 32768) {
		{
			...
			dispatch(fork,
				 [=] { fork_join_sort(begin, begin + n / 2); });
			dispatch(fork,
				 [=] { fork_join_sort(begin + n / 2, end); });
		}
