...
struct custom_tracking {
	...
	static void creation(boost::asio::execution_context & /*ctx*/,
			     tracked_handler &h, const char *object_type,
			     void * /*object*/, std::uintmax_t native_handle,
			     const char *op_name)
	{
	...
	struct completion {
		explicit completion(const tracked_handler &h)
		    : handler_(h), next_(*current_completion())
		{
		...
		void invocation_end()
		{
			std::printf("Leaving handler %" PRIuMAX
				    " in tree %" PRIuMAX "\n",
				    handler_.handler_id_, handler_.tree_id_);
		}
	...
	static void
	reactor_registration(boost::asio::execution_context &context,
	...
	static void
	reactor_deregistration(boost::asio::execution_context &context,
			       uintmax_t native_handle, uintmax_t registration)
	{
	...
	{
		std::printf(
		    "Reactor readiness for registration = %" PRIuMAX
		    ", events =%s%s%s\n",
		    registration,
