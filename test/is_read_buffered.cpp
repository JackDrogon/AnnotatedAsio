...
public:
	...
	template <typename Const_Buffers> size_t write(const Const_Buffers &)
	{
		return 0;
	}
	...
	template <typename Mutable_Buffers> size_t read(const Mutable_Buffers &)
	{
		return 0;
	}
...
void is_read_buffered_test()
{
	...
	BOOST_ASIO_CHECK(
	    !!boost::asio::is_read_buffered<boost::asio::buffered_read_stream<
		 boost::asio::ip::tcp::socket>>::value);

	BOOST_ASIO_CHECK(
	    !boost::asio::is_read_buffered<boost::asio::buffered_write_stream<
		boost::asio::ip::tcp::socket>>::value);

	BOOST_ASIO_CHECK(
	    !!boost::asio::is_read_buffered<boost::asio::buffered_stream<
		 boost::asio::ip::tcp::socket>>::value);
...
BOOST_ASIO_TEST_SUITE("is_read_buffered",
