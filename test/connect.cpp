...
void test_connect_range_cond()
{
	...
	try {
		result =
		    boost::asio::connect(socket, endpoints, legacy_true_cond_1);
		BOOST_ASIO_CHECK(false);
	} catch (boost::system::system_error &e) {
		BOOST_ASIO_CHECK(e.code() == boost::asio::error::not_found);
	}
...
void test_connect_range_cond_ec()
{
	...
	result =
	    boost::asio::connect(socket, endpoints, legacy_true_cond_1, ec);
	BOOST_ASIO_CHECK(result == boost::asio::ip::tcp::endpoint());
	BOOST_ASIO_CHECK(ec == boost::asio::error::not_found);

	result =
	    boost::asio::connect(socket, endpoints, legacy_true_cond_2(), ec);
	BOOST_ASIO_CHECK(result == boost::asio::ip::tcp::endpoint());
	BOOST_ASIO_CHECK(ec == boost::asio::error::not_found);
...
void test_connect_iter()
{
	...
	boost::asio::ip::tcp::socket socket(io_context);
	std::vector<boost::asio::ip::tcp::endpoint> endpoints;
	const std::vector<boost::asio::ip::tcp::endpoint> &cendpoints =
	    endpoints;
	...
	result =
	    boost::asio::connect(socket, cendpoints.begin(), cendpoints.end());
...
void test_connect_iter_cond()
{
	...
	try {
		result =
		    boost::asio::connect(socket, cendpoints.begin(),
...
void test_async_connect_range_cond()
{
	...
	boost::asio::async_connect(
	    socket, endpoints, legacy_true_cond_1,
	    bindns::bind(range_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == boost::asio::ip::tcp::endpoint());
	...
	boost::asio::async_connect(
	    socket, endpoints, legacy_true_cond_2(),
	    bindns::bind(range_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == boost::asio::ip::tcp::endpoint());
...
void test_async_connect_iter()
{
	...
	boost::asio::async_connect(
	    socket, cendpoints.begin(), cendpoints.end(),
...
void test_async_connect_iter_cond()
{
	...
	boost::asio::async_connect(
	    socket, cendpoints.begin(), cendpoints.end(), true_cond_1,
	    bindns::bind(iter_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == cendpoints.end());
	...
	boost::asio::async_connect(
	    socket, cendpoints.begin(), cendpoints.end(), true_cond_2(),
	    bindns::bind(iter_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == cendpoints.end());
	...
	boost::asio::async_connect(
	    socket, cendpoints.begin(), cendpoints.end(), legacy_true_cond_1,
	    bindns::bind(iter_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == cendpoints.end());
	...
	boost::asio::async_connect(
	    socket, cendpoints.begin(), cendpoints.end(), legacy_true_cond_2(),
	    bindns::bind(iter_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == cendpoints.end());
	...
	boost::asio::async_connect(
	    socket, cendpoints.begin(), cendpoints.end(), false_cond,
	    bindns::bind(iter_handler, _1, _2, &ec, &result));
	io_context.restart();
	io_context.run();
	BOOST_ASIO_CHECK(result == cendpoints.end());
...
BOOST_ASIO_TEST_SUITE(
	...
	BOOST_ASIO_TEST_CASE(test_connect_range_cond_ec) BOOST_ASIO_TEST_CASE(
	    test_connect_iter) BOOST_ASIO_TEST_CASE(test_connect_iter_ec)
	    BOOST_ASIO_TEST_CASE(test_connect_iter_cond) BOOST_ASIO_TEST_CASE(
