...
void decrement_to_zero(io_context *ioc, int *count)
{
	if (*count > 0) {
		...
		boost::asio::post(*ioc,
				  bindns::bind(decrement_to_zero, ioc, count));
...
void nested_decrement_to_zero(io_context *ioc, int *count)
{
	if (*count > 0) {
		...
		boost::asio::dispatch(
		    *ioc, bindns::bind(nested_decrement_to_zero, ioc, count));
...
void sleep_increment(io_context *ioc, int *count)
{
	...
	if (++(*count) < 3)
		boost::asio::post(*ioc,
				  bindns::bind(sleep_increment, ioc, count));
}
...
void io_context_test()
{
	...
	BOOST_ASIO_CHECK(!ioc.stopped());
	boost::asio::post(ioc,
			  bindns::bind(start_sleep_increments, &ioc, &count));
	boost::asio::post(ioc,
			  bindns::bind(start_sleep_increments, &ioc, &count2));
	...
	io_context ioc2;
	boost::asio::dispatch(
	    ioc, boost::asio::bind_executor(
		     ioc2, bindns::bind(decrement_to_zero, &ioc2, &count)));
	ioc.restart();
	BOOST_ASIO_CHECK(!ioc.stopped());
	ioc.run();
...
BOOST_ASIO_TEST_SUITE("io_context",
