...
void test()
{
	...
	try {
		...
		serial_port::native_handle_type native_port1 =
		    port1.native_handle();
		...
		serial_port::lowest_layer_type &lowest_layer =
		    port1.lowest_layer();
		(void)lowest_layer;

		const serial_port &port5 = port1;
		const serial_port::lowest_layer_type &lowest_layer2 =
		    port5.lowest_layer();
		(void)lowest_layer2;
		...
		serial_port::native_handle_type native_port2 =
		    port1.native_handle();
		port1.assign(native_port2);
		serial_port::native_handle_type native_port3 =
		    port1.native_handle();
		...
		serial_port::native_handle_type native_port4 =
		    port1.native_handle();
		(void)native_port4;
		...
		port1.async_write_some(buffer(mutable_char_buffer),
				       write_some_handler());
		port1.async_write_some(buffer(const_char_buffer),
				       write_some_handler());
		int i1 =
		    port1.async_write_some(buffer(mutable_char_buffer), lazy);
		(void)i1;
		int i2 =
		    port1.async_write_some(buffer(const_char_buffer), lazy);
		(void)i2;

		port1.read_some(buffer(mutable_char_buffer));
		port1.read_some(buffer(mutable_char_buffer), ec);

		port1.async_read_some(buffer(mutable_char_buffer),
				      read_some_handler());
		int i3 =
		    port1.async_read_some(buffer(mutable_char_buffer), lazy);
		(void)i3;
	} catch (std::exception &) {
	}
