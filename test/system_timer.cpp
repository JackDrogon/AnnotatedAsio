...
void increment_if_not_cancelled(int *count, const boost::system::error_code &ec)
{
	if (!ec)
		++(*count);
}
...
void system_timer_cancel_test()
{
	...
	struct timer {
		...
		{
			t.expires_at(
			    (boost::asio::system_timer::time_point::max)());
		}
	} timers[50];
...
void system_timer_custom_allocation_test()
{
	...
	for (int i = 0; i < 50; ++i) {
		timers[i].t.expires_at(
		    (boost::asio::system_timer::time_point::max)());
		timers[i].t.async_wait(
		    custom_allocation_timer_handler(&allocation_count));
	}

	for (int i = 50; i < 100; ++i) {
		timers[i].t.expires_at(
		    (boost::asio::system_timer::time_point::min)());
		timers[i].t.async_wait(
		    custom_allocation_timer_handler(&allocation_count));
	}
...
BOOST_ASIO_TEST_SUITE("system_timer", BOOST_ASIO_TEST_CASE(null_test))
