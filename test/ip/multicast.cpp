...
void test()
{
	...
	try {
		...
		ip::multicast::outbound_interface outbound_interface2(
		    address_v4);
...
void test()
{
	...
	if (have_v4) {
		...
		if (!ec) {
			...
			ip::multicast::leave_group leave_group(
			    multicast_address_v4);
	...
	if (have_v4) {
		...
		BOOST_ASIO_CHECK_MESSAGE(!ec, ec.value()
						  << ", " << ec.message());
	}
