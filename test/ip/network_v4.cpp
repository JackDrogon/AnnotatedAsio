...
void test()
{
	...
	// prefix length
	BOOST_ASIO_CHECK(make_network_v4("193.99.144.80/24").prefix_length() ==
			 24);
	BOOST_ASIO_CHECK(
	    network_v4(make_address_v4("193.99.144.80"), 24).prefix_length() ==
	    24);
	...
	BOOST_ASIO_CHECK(
	    make_network_v4("192.168.78.30/20").address().to_string() ==
	    "192.168.78.30");
	...
	BOOST_ASIO_CHECK(
	    make_network_v4("192.168.0.0/24")
		.is_subnet_of(make_network_v4("192.168.192.168/16")));
	BOOST_ASIO_CHECK(
	    make_network_v4("192.168.0.192/24")
		.is_subnet_of(make_network_v4("192.168.192.168/16")));
	...
	// iterator
	BOOST_ASIO_CHECK(
	    std::distance(net12.hosts().begin(), net12.hosts().end()) == 254);
	BOOST_ASIO_CHECK(*net12.hosts().begin() ==
			 make_address_v4("192.168.0.1"));
	...
	BOOST_ASIO_CHECK(
	    std::distance(net13.hosts().begin(), net13.hosts().end()) == 14);
	BOOST_ASIO_CHECK(*net13.hosts().begin() ==
			 make_address_v4("192.168.1.1"));
	...
	BOOST_ASIO_CHECK(
	    std::distance(net14.hosts().begin(), net14.hosts().end()) == 14);
	BOOST_ASIO_CHECK(*net14.hosts().begin() ==
			 make_address_v4("192.168.1.17"));
