...
void test()
{
	...
	try {
		...
		spp::socket::lowest_layer_type &lowest_layer =
		...
		spp::socket::native_handle_type native_socket4 =
		...
		socket1.async_receive(buffer(mutable_char_buffer), out_flags,
				      receive_handler);
		socket1.async_receive(null_buffers(), out_flags,
				      receive_handler);
		socket1.async_receive(buffer(mutable_char_buffer), in_flags,
				      out_flags, receive_handler);
		socket1.async_receive(null_buffers(), in_flags, out_flags,
				      receive_handler);
	} catch (std::exception &) {
	}
}
