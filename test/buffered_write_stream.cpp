...
typedef boost::asio::buffered_write_stream<boost::asio::ip::tcp::socket>
    stream_type;
...
void test_sync_operations()
{
	...
	const boost::asio::mutable_buffer read_buf =
	...
	BOOST_ASIO_CHECK(memcmp(write_data, read_data, sizeof(write_data)) ==
			 0);
	...
	bytes_read =
	    client_socket.read_some(boost::asio::buffer(read_buf), error);
...
void test_async_operations()
{
	...
	server_socket.close();
	client_socket.async_read_some(boost::asio::buffer(read_buf),
				      handle_read_eof);
}

BOOST_ASIO_TEST_SUITE("buffered_write_stream",
