...
public:
	...
	test_stream(boost::asio::io_context &io_context)
	    : io_context_(io_context), length_(max_length), position_(0),
	...
	bool check_buffers(Iterator begin, Iterator end, size_t length)
	{
		...
		for (; iter != end && checked_length < length; ++iter) {
			...
			if (memcmp(data_ + checked_length, iter->data(),
				   buffer_length) != 0)
				return false;
	...
	size_t write_some(const Const_Buffers &buffers)
	{
		size_t n = boost::asio::buffer_copy(
		    boost::asio::buffer(data_, length_) + position_, buffers,
		    next_write_length_);
		position_ += n;
		return n;
	}
...
void test_3_arg_const_buffer_write()
{
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_at_least(1));
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_at_least(10));
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_at_least(42));
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_exactly(1));
	BOOST_ASIO_CHECK(bytes_transferred == 1);
	BOOST_ASIO_CHECK(s.check_buffers(buffers, 1));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_exactly(10));
	BOOST_ASIO_CHECK(bytes_transferred == 10);
	BOOST_ASIO_CHECK(s.check_buffers(buffers, 10));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_exactly(42));
	BOOST_ASIO_CHECK(bytes_transferred == 42);
	BOOST_ASIO_CHECK(s.check_buffers(buffers, 42));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, old_style_transfer_all);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
...
void test_4_arg_const_buffer_write()
{
	...
	boost::system::error_code error;
	size_t bytes_transferred =
	    boost::asio::write(s, buffers, boost::asio::transfer_all(), error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	bytes_transferred = boost::asio::write(
	    s, buffers, boost::asio::transfer_at_least(1), error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	error = boost::system::error_code();
	bytes_transferred = boost::asio::write(
	    s, buffers, boost::asio::transfer_at_least(10), error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	error = boost::system::error_code();
	bytes_transferred = boost::asio::write(
	    s, buffers, boost::asio::transfer_at_least(42), error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	error = boost::system::error_code();
	bytes_transferred = boost::asio::write(
	    s, buffers, boost::asio::transfer_exactly(1), error);
	BOOST_ASIO_CHECK(bytes_transferred == 1);
	BOOST_ASIO_CHECK(s.check_buffers(buffers, 1));
	BOOST_ASIO_CHECK(!error);
	...
	error = boost::system::error_code();
	bytes_transferred = boost::asio::write(
	    s, buffers, boost::asio::transfer_exactly(10), error);
	BOOST_ASIO_CHECK(bytes_transferred == 10);
	BOOST_ASIO_CHECK(s.check_buffers(buffers, 10));
	BOOST_ASIO_CHECK(!error);
	...
	error = boost::system::error_code();
	bytes_transferred = boost::asio::write(
	    s, buffers, boost::asio::transfer_exactly(42), error);
	BOOST_ASIO_CHECK(bytes_transferred == 42);
	BOOST_ASIO_CHECK(s.check_buffers(buffers, 42));
	BOOST_ASIO_CHECK(!error);
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, old_style_transfer_all, error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write(s, buffers, short_transfer, error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
...
void test_3_arg_const_buffer_async_write()
{
	...
	s.reset();
	int i =
	    boost::asio::async_write(s, buffers, archetypes::lazy_handler());
...
void test_4_arg_const_buffer_async_write()
{
	...
	called = false;
	boost::asio::async_write(
	    s, buffers, boost::asio::transfer_exactly(1),
	    bindns::bind(async_write_handler, _1, _2, 1, &called));
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(called);
...
void test_4_arg_streambuf_async_write()
{
	...
	called = false;
	boost::asio::async_write(
	    s, sb, boost::asio::transfer_exactly(1),
	    bindns::bind(async_write_handler, _1, _2, 1, &called));
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(called);
...
BOOST_ASIO_TEST_SUITE(
    "write",
    BOOST_ASIO_TEST_CASE(test_2_arg_zero_buffers_write) BOOST_ASIO_TEST_CASE(
	test_2_arg_const_buffer_write) BOOST_ASIO_TEST_CASE(test_2_arg_mutable_buffer_write)
	BOOST_ASIO_TEST_CASE(test_2_arg_vector_buffers_write) BOOST_ASIO_TEST_CASE(
	    test_3_arg_nothrow_zero_buffers_write) BOOST_ASIO_TEST_CASE(test_3_arg_nothrow_const_buffer_write)
	    BOOST_ASIO_TEST_CASE(test_3_arg_nothrow_mutable_buffer_write) BOOST_ASIO_TEST_CASE(
		test_3_arg_nothrow_vector_buffers_write)
		BOOST_ASIO_TEST_CASE(test_3_arg_const_buffer_write) BOOST_ASIO_TEST_CASE(
		    test_3_arg_mutable_buffer_write) BOOST_ASIO_TEST_CASE(test_3_arg_vector_buffers_write)
		    BOOST_ASIO_TEST_CASE(test_4_arg_const_buffer_write) BOOST_ASIO_TEST_CASE(
			test_4_arg_mutable_buffer_write) BOOST_ASIO_TEST_CASE(test_4_arg_vector_buffers_write)
			BOOST_ASIO_TEST_CASE(test_3_arg_const_buffer_async_write) BOOST_ASIO_TEST_CASE(
			    test_3_arg_mutable_buffer_async_write)
			    BOOST_ASIO_TEST_CASE(
				test_3_arg_boost_array_buffers_async_write)
				BOOST_ASIO_TEST_CASE(
				    test_3_arg_std_array_buffers_async_write)
				    BOOST_ASIO_TEST_CASE(
					test_3_arg_vector_buffers_async_write)
					BOOST_ASIO_TEST_CASE(
					    test_3_arg_streambuf_async_write)
					    BOOST_ASIO_TEST_CASE(
						test_4_arg_const_buffer_async_write)
						BOOST_ASIO_TEST_CASE(
						    test_4_arg_mutable_buffer_async_write)
						    BOOST_ASIO_TEST_CASE(
							test_4_arg_boost_array_buffers_async_write)
							BOOST_ASIO_TEST_CASE(
							    test_4_arg_std_array_buffers_async_write)
							    BOOST_ASIO_TEST_CASE(
								test_4_arg_vector_buffers_async_write)
								BOOST_ASIO_TEST_CASE(
								    test_4_arg_streambuf_async_write))
