...
void test()
{
	...
	try {
		...
		posix::stream_descriptor::executor_type ex =
		    descriptor1.get_executor();
		(void)ex;
		...
		const posix::stream_descriptor::lowest_layer_type
		...
		posix::stream_descriptor::native_handle_type
		    native_descriptor3 = descriptor1.native_handle();
		(void)native_descriptor3;

		posix::stream_descriptor::native_handle_type
		    native_descriptor4 = descriptor1.release();
		...
		descriptor1.async_wait(posix::descriptor_base::wait_read,
				       &wait_handler);
		int i1 = descriptor1.async_wait(
		    posix::descriptor_base::wait_write, lazy);
		(void)i1;
		...
		descriptor1.async_write_some(null_buffers(),
					     write_some_handler);
		int i2 = descriptor1.async_write_some(
		    buffer(mutable_char_buffer), lazy);
		(void)i2;
		int i3 = descriptor1.async_write_some(buffer(const_char_buffer),
						      lazy);
		(void)i3;
		...
		descriptor1.async_read_some(buffer(mutable_char_buffer),
					    read_some_handler);
		descriptor1.async_read_some(null_buffers(), read_some_handler);
		int i5 = descriptor1.async_read_some(
		    buffer(mutable_char_buffer), lazy);
		(void)i5;
