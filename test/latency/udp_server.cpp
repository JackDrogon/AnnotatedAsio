...
public:
	...
	void operator()(boost::system::error_code ec, std::size_t n = 0)
	{
		reenter(this) for (;;)
		{
			yield socket_.async_receive_from(
			    boost::asio::buffer(buffer_), sender_, ref(this));

			if (!ec) {
				for (std::size_t i = 0; i < n; ++i)
					buffer_[i] = ~buffer_[i];
				socket_.send_to(boost::asio::buffer(buffer_, n),
						sender_, 0, ec);
			}
		}
	}
	...
	private:
		...
		friend void asio_handler_deallocate(void *p, std::size_t n,
						    ref *r)
		{
...
{
	if (argc != 5) {
		std::fprintf(stderr, "Usage: udp_server <port1> <nports> "
				     "<bufsize> {spin|block}\n");
		return 1;
	}
	...
	for (unsigned short i = 0; i < num_ports; ++i) {
		...
		boost::shared_ptr<udp_server> s(
		    new udp_server(io_context, port, buf_size));
		servers.push_back(s);
