...
public:
	...
	test_random_access_device(boost::asio::io_context &io_context)
	    : io_context_(io_context), length_(max_length),
	...
	void next_write_length(size_t length) { next_write_length_ = length; }
			     ...
			     const Const_Buffers &buffers)
	{
		return boost::asio::buffer_copy(
		    boost::asio::buffer(data_, length_) + offset, buffers,
		    next_write_length_);
	}
...
void test_3_arg_mutable_buffer_write_at()
{
	...
	BOOST_ASIO_CHECK(
	    s.check_buffers(0, buffers, sizeof(mutable_write_data)));

	s.reset();
	bytes_transferred = boost::asio::write_at(s, 1234, buffers);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(mutable_write_data));
	BOOST_ASIO_CHECK(
	    s.check_buffers(1234, buffers, sizeof(mutable_write_data)));
...
void test_4_arg_const_buffer_write_at()
{
	...
	s.reset();
	size_t bytes_transferred =
	    boost::asio::write_at(s, 0, buffers, boost::asio::transfer_all());
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, sizeof(write_data)));
	...
	s.reset();
	bytes_transferred = boost::asio::write_at(
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 0, buffers, old_style_transfer_all);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, sizeof(write_data)));

	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 1234, buffers, old_style_transfer_all);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(1234, buffers, sizeof(write_data)));
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 0, buffers, short_transfer);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, sizeof(write_data)));

	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 1234, buffers, short_transfer);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(1234, buffers, sizeof(write_data)));
...
void test_5_arg_const_buffer_write_at()
{
	...
	s.reset();
	boost::system::error_code error;
	size_t bytes_transferred = boost::asio::write_at(
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 0, buffers, old_style_transfer_all, error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	s.reset();
	bytes_transferred = boost::asio::write_at(
	    s, 1234, buffers, old_style_transfer_all, error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(1234, buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 0, buffers, short_transfer, error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
	...
	s.reset();
	bytes_transferred =
	    boost::asio::write_at(s, 1234, buffers, short_transfer, error);
	BOOST_ASIO_CHECK(bytes_transferred == sizeof(write_data));
	BOOST_ASIO_CHECK(s.check_buffers(1234, buffers, sizeof(write_data)));
	BOOST_ASIO_CHECK(!error);
...
void test_4_arg_const_buffer_async_write_at()
{
	...
	boost::asio::async_write_at(s, 0, buffers,
				    bindns::bind(async_write_handler, _1, _2,
...
void test_4_arg_streambuf_async_write_at()
{
	...
	sb.sputn(write_data, sizeof(write_data));
	int i =
	    boost::asio::async_write_at(s, 0, sb, archetypes::lazy_handler());
	BOOST_ASIO_CHECK(i == 42);
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, sizeof(write_data)));
...
void test_5_arg_const_buffer_async_write_at()
{
	...
	s.reset();
	bool called = false;
	boost::asio::async_write_at(s, 0, buffers, boost::asio::transfer_all(),
				    bindns::bind(async_write_handler, _1, _2,
						 sizeof(write_data), &called));
	ioc.restart();
	ioc.run();
	...
	boost::asio::async_write_at(
	    s, 0, buffers, boost::asio::transfer_at_least(1),
	    bindns::bind(async_write_handler, _1, _2, 1, &called));
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(called);
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, 1));
	...
	boost::asio::async_write_at(
	    s, 0, buffers, boost::asio::transfer_at_least(1),
	    bindns::bind(async_write_handler, _1, _2, 10, &called));
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(called);
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, 10));
	...
	boost::asio::async_write_at(
	    s, 0, buffers, boost::asio::transfer_at_least(42),
	    bindns::bind(async_write_handler, _1, _2, 42, &called));
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(called);
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, 42));
	...
	boost::asio::async_write_at(
	    s, 0, buffers, boost::asio::transfer_at_least(42),
	    bindns::bind(async_write_handler, _1, _2, 50, &called));
	ioc.restart();
	ioc.run();
	BOOST_ASIO_CHECK(called);
	BOOST_ASIO_CHECK(s.check_buffers(0, buffers, 50));
...
void test_5_arg_mutable_buffer_async_write_at()
{
	...
	called = false;
	boost::asio::async_write_at(
	    s, 1234, buffers, boost::asio::transfer_all(),
	    bindns::bind(async_write_handler, _1, _2,
...
void test_5_arg_streambuf_async_write_at()
{
	...
	sb.sputn(write_data, sizeof(write_data));
	bool called = false;
	boost::asio::async_write_at(s, 0, sb, boost::asio::transfer_all(),
				    bindns::bind(async_write_handler, _1, _2,
						 sizeof(write_data), &called));
	ioc.restart();
	ioc.run();
	...
	sb.sputn(write_data, sizeof(write_data));
	called = false;
	boost::asio::async_write_at(s, 1234, sb, boost::asio::transfer_all(),
				    bindns::bind(async_write_handler, _1, _2,
						 sizeof(write_data), &called));
	ioc.restart();
	ioc.run();
	...
	sb.sputn(write_data, sizeof(write_data));
	called = false;
	boost::asio::async_write_at(s, 0, sb, boost::asio::transfer_at_least(1),
				    bindns::bind(async_write_handler, _1, _2,
						 sizeof(write_data), &called));
	ioc.restart();
	ioc.run();
...
BOOST_ASIO_TEST_SUITE(
    "write_at",
    BOOST_ASIO_TEST_CASE(test_3_arg_const_buffer_write_at) BOOST_ASIO_TEST_CASE(
	test_3_arg_mutable_buffer_write_at) BOOST_ASIO_TEST_CASE(test_3_arg_vector_buffers_write_at)
	BOOST_ASIO_TEST_CASE(test_4_arg_nothrow_const_buffer_write_at) BOOST_ASIO_TEST_CASE(
	    test_4_arg_nothrow_mutable_buffer_write_at)
	    BOOST_ASIO_TEST_CASE(test_4_arg_nothrow_vector_buffers_write_at) BOOST_ASIO_TEST_CASE(
		test_4_arg_const_buffer_write_at) BOOST_ASIO_TEST_CASE(test_4_arg_mutable_buffer_write_at)
		BOOST_ASIO_TEST_CASE(test_4_arg_vector_buffers_write_at) BOOST_ASIO_TEST_CASE(
		    test_5_arg_const_buffer_write_at)
		    BOOST_ASIO_TEST_CASE(test_5_arg_mutable_buffer_write_at) BOOST_ASIO_TEST_CASE(
			test_5_arg_vector_buffers_write_at)
			BOOST_ASIO_TEST_CASE(test_4_arg_const_buffer_async_write_at) BOOST_ASIO_TEST_CASE(
			    test_4_arg_mutable_buffer_async_write_at)
			    BOOST_ASIO_TEST_CASE(
				test_4_arg_boost_array_buffers_async_write_at)
				BOOST_ASIO_TEST_CASE(
				    test_4_arg_std_array_buffers_async_write_at)
				    BOOST_ASIO_TEST_CASE(
					test_4_arg_vector_buffers_async_write_at)
					BOOST_ASIO_TEST_CASE(
					    test_4_arg_streambuf_async_write_at)
					    BOOST_ASIO_TEST_CASE(
						test_5_arg_const_buffer_async_write_at)
						BOOST_ASIO_TEST_CASE(
						    test_5_arg_mutable_buffer_async_write_at)
						    BOOST_ASIO_TEST_CASE(
							test_5_arg_boost_array_buffers_async_write_at)
							BOOST_ASIO_TEST_CASE(
							    test_5_arg_std_array_buffers_async_write_at)
							    BOOST_ASIO_TEST_CASE(
								test_5_arg_vector_buffers_async_write_at)
								BOOST_ASIO_TEST_CASE(
								    test_5_arg_streambuf_async_write_at))
